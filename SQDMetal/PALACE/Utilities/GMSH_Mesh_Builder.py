...
class GMSH_Mesh_Builder:
    ...
    def build_mesh(self):
        ...
        for m, cur_fine_mesh in enumerate(self._fine_meshes):
            ...
            if "region" in cur_fine_mesh:
                gmsh.model.mesh.field.setNumbers(
                    2 * m + 1, "SurfacesList", [x[1] for x in cur_fine_mesh["region"]]
                )
            elif "path" in cur_fine_mesh:
                gmsh.model.mesh.field.setNumbers(
                    2 * m + 1, "CurvesList", cur_fine_mesh["path"]
                )
            else:
                assert False, (
                    "The fine-mesh parameter does not have a supported object to reference/mesh - e.g. a region or path key..."
                )
            #
            gmsh.model.mesh.field.setNumber(
                2 * m + 1,
                "Sampling",
                self.user_options.get("gmsh_dist_func_discretisation", 150),
            )
            ...
            gmsh.model.mesh.field.setNumber(
                thresh_field_id, "SizeMin", cur_fine_mesh["mesh_min"]
            )  # minimum size of mesh element
            gmsh.model.mesh.field.setNumber(
                thresh_field_id, "SizeMax", cur_fine_mesh["mesh_max"]
            )  # maximum size of mesh element
            gmsh.model.mesh.field.setNumber(
                thresh_field_id, "DistMin", cur_fine_mesh["taper_dist_min"]
            )  # distance from the gmsh surface to keep minimum mesh element size
            gmsh.model.mesh.field.setNumber(
                thresh_field_id, "DistMax", cur_fine_mesh["taper_dist_max"]
            )  # how far from the element until the max element size can be implemented
        ...
        gmsh.option.setNumber(
            "Mesh.Algorithm3D", 10
        )  # HXT seems to be producing less slivers near curvature in the design

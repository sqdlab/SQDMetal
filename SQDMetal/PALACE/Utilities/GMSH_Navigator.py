...
class GMSH_Navigator:
    def __init__(self, mesh_file, units=0.001, gmsh_verbosity=1):
        ...
        self.physical_group_names_entities = [
            (
                gmsh.model.get_physical_name(*x),
                gmsh.model.getEntitiesForPhysicalGroup(*x),
                x,
            )
            for x in lePhysGroups
        ]
        #
        self.leCols = plt.rcParams["axes.prop_cycle"].by_key()["color"]
        self.leColsRGB = [
            tuple(int(round(v * 255)) for v in matplotlib.colors.to_rgb(c))
            for c in self.leCols
        ]
    ...
    def export_to_png(
        self, filt_names=["metal", "rf_port", "dielectric_gaps"], file_path=""
    ):
        ...
        for m, cur_phys_group in enumerate(self.physical_group_names_entities):
            ...
            filtNodeTags, _ = gmsh.model.mesh.getNodesForPhysicalGroup(
                *cur_phys_group[2]
            )
            filtEdges = [
                [leNodes[x[0]][:2] * self._units, leNodes[x[1]][:2] * self._units]
                for x in edges
                if x[0] in filtNodeTags and x[1] in filtNodeTags
            ]
            ...
            lc = mc.LineCollection(
                filtEdges, linewidths=1, color=self.leCols[m % len(self.leCols)]
            )
    ...
    def plot_mesh(self, filt_names=["metal", "rf_port", "dielectric_gaps"], **kwargs):
        ...
        for m, cur_phys_group in enumerate(self.physical_group_names_entities):
            ...
            face_triangles = [
                [normalised_indices[pt] for pt in x]
                for x in faces
                if x[0] in filtNodeTags
                and x[1] in filtNodeTags
                and x[2] in filtNodeTags
            ]

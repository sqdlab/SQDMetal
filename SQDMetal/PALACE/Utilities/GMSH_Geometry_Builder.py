...
class GMSH_Geometry_Builder:
    ...
    def construct_geometry_in_GMSH(self, metallic_layers, ground_plane, sim_constructs, fine_meshes, fuse_threshold, **kwargs):
        ...
        for i,metal in enumerate(metal_list):
            ...
            metal_physical_group = gmsh.model.addPhysicalGroup(2, [metal[1]], name = metal_name)
        ...
        leDielectric = gmsh.model.addPhysicalGroup(3, [chip_base], name = 'dielectric_substrate')
    ...
    def _process_qiskit_geometries(self, metallic_layers, ground_plane, fuse_threshold, **kwargs):
        ...
        if not ground_plane['omit'] and not ground_plane_poly.is_empty:
            metal_polys[0] = ground_plane_poly
            num_polys += 1
        ...
        for poly in metal_polys:
            metal_polys_new += ShapelyEx.shapely_to_list(metal_polys[poly])
        ...
        def get_poly_cardinality(poly):
            if isinstance(poly, shapely.geometry.multipolygon.MultiPolygon):
                ...
                return 1
        new_polys = [metal_polys_new[0]]
        for m in range(1, len(metal_polys_new)):
            found_match = False
            for n in range(len(new_polys)):
                cur_check = ShapelyEx.fuse_polygons_threshold([metal_polys_new[m], new_polys[n]], fuse_threshold)
                if get_poly_cardinality(cur_check) < get_poly_cardinality(metal_polys_new[m]) + get_poly_cardinality(new_polys[n]):
                    new_polys[n] = cur_check
                    found_match = True
                    break
            if not found_match:
                new_polys.append(metal_polys_new[m])

        all_metals = shapely.unary_union(new_polys)
        ...
        dielectric_gaps = shapely.difference(substrate, all_metals)

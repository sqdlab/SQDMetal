...
class GMSH_Geometry_Builder:
    ...
    def construct_geometry_in_GMSH(
        self,
        metallic_layers,
        ground_plane,
        sim_constructs,
        ...
        **kwargs,
        ...
        # and the metals
        metals, dielectric_gaps = self._geom_processor.process_layers(
            metallic_layers,
            ground_plane,
            fuse_threshold=fuse_threshold,
            fillet_resolution=self.fillet_resolution,
            unit_conv=1e-3,
            **kwargs,
        )  # It's in mm...
        ...
        metal_list = self._create_gmsh_geometry_from_shapely_polygons(
            metals
        )  # create all metal traces
        dielectric_gap_list = self._create_gmsh_geometry_from_shapely_polygons(
            dielectric_gaps
        )  # create all the gaps between the metal traces and the ground plane
        #
        fragment_list = []
        ...
        for cur_sim_poly in sim_constructs:
            sim_poly = self._draw_polygon_in_GMSH_from_coords(
                np.array(cur_sim_poly[1])[:-1] * 1e3
            )  # i.e. ignore closed loop part as function closes it automatically and use mm
            sim_poly, leMap = gmsh.model.occ.cut(
                [(2, sim_poly)],
                metal_cut_list_for_ports,
                removeObject=True,
                removeTool=False,
            )
            gmsh.model.occ.synchronize()
        ...
        if len(aux_list) > 0:
            ...
            dielectric_gap_list, lemap = gmsh.model.occ.cut(
                dielectric_gap_list_for_cut,
                cut_list,
                removeObject=True,
                removeTool=False,
            )
        ...
        chip, chip_map = gmsh.model.occ.fragment(
            [(3, tag3D_chip_base)], fragment_list, removeObject=True, removeTool=True
        )
        surface_remappings = {}
        ...
        for cur_port in cur_port_names:
            ...
            lePortPolys[cur_port] = gmsh.model.addPhysicalGroup(
                2, [x[1] for x in cur_port_surfaces_final], name=cur_port
            )
        #
        gmsh.model.occ.synchronize()
        gmsh.model.geo.synchronize()
        ...
        chip_and_air_box, chip_and_air_box_map = gmsh.model.occ.fragment(
            [(3, tag3D_chip_base)], [(3, air_box)], removeObject=True, removeTool=True
        )
        gmsh.model.occ.synchronize()
        gmsh.model.geo.synchronize()
        ...
        tag3D_air_box = [x for x in chip_and_air_box_map[1] if x[1] != tag3D_chip_base][
            0
        ][1]
        ...
        leFarField = gmsh.model.addPhysicalGroup(
            2, far_field_surfaces, name="far_field"
        )
        #
        gmsh.model.geo.synchronize()
        gmsh.model.occ.synchronize()
        ...
        for cur_fine_mesh in fine_meshes:
            if cur_fine_mesh["type"] == "box":
                ...
                # Get it in mm...
                cur_mesh_attrb["region"] = (
                    self._create_gmsh_geometry_from_shapely_polygons(
                        [ShapelyEx.rectangle(x1 * 1e3, y1 * 1e3, x2 * 1e3, y2 * 1e3)]
                    )
                )
                ...
                assert isinstance(self._geom_processor, GeomQiskitMetal), (
                    "The geometry must be in Qiskit-Metal format to use comp_bounds..."
                )
                cur_mesh_attrb = {}
                comp_outlines = QUtilities.get_perimetric_polygons(
                    self._geom_processor.design,
                    cur_fine_mesh["list_comp_names"],
                    fuse_threshold=fuse_threshold,
                    resolution=self.fillet_resolution,
                    unit_conv=1,
                    metals_only=cur_fine_mesh["metals_only"],
                )  # Get it in mm...
                cur_mesh_attrb["region"] = (
                    self._create_gmsh_geometry_from_shapely_polygons(comp_outlines)
                )
                cur_mesh_attrb["mesh_min"] = cur_fine_mesh["min_size"] * 1e3
                cur_mesh_attrb["mesh_max"] = cur_fine_mesh["max_size"] * 1e3
                cur_mesh_attrb["taper_dist_min"] = cur_fine_mesh["taper_dist_min"] * 1e3
                ...
                # Get it in mm...
                lePoints = [
                    gmsh.model.geo.addPoint(x[0] * 1e3, x[1] * 1e3, 0)
                    for x in cur_fine_mesh["path"]
                ]
                leLines = [
                    gmsh.model.geo.addLine(lePoints[m - 1], lePoints[m])
                    for m in range(1, len(lePoints))
                ]
                gmsh.model.occ.synchronize()
                gmsh.model.geo.synchronize()
                #
        ...
        if full_3D_params is None or full_3D_params["metal_thickness"] == 0:
            for m, cur_metal_entities in enumerate(metal_cap_physical_group):
                ...
                metal_physical_group = gmsh.model.addPhysicalGroup(
                    2, cur_metal_entities, name=metal_name
                )
                lemetals_physgrps.append(metal_physical_group)
                metal_cap_names.append(
                    metal_name
                )  # TODO: Look into why is this even stored?
        else:
            ...
            for cur_metal_entities in metal_cap_physical_group:
                ex_metals = gmsh.model.occ.extrude(
                    [(2, x) for x in cur_metal_entities], 0, 0, metal_thickness
                )
                gmsh.model.occ.synchronize()
                gmsh.model.geo.synchronize()
            ...
            for m in range(len(new_extrusion_metals)):
                ...
                tags_3D = [
                    cur_tag[1]
                    for cur_tag in chip_and_air_box_map[m + 1]
                    if cur_tag[0] == 3
                ]
                cur_metal_surfaces = []
                for cur_tag_3D in tags_3D:
                    # Note that here getAdjacencies returns 4D and 2D entities (empty list for former)
                    cur_metal_surfaces += gmsh.model.getAdjacencies(3, cur_tag_3D)[
                        1
                    ].tolist()

                metal_name = "metal_" + str(m)
                metal_physical_group = gmsh.model.addPhysicalGroup(
                    2, cur_metal_surfaces, name=metal_name
                )
                lemetals_physgrps.append(metal_physical_group)
                metal_cap_names.append(
                    metal_name
            ...
            air_box_tags = [
                x[1]
                for x in chip_and_air_box_map[0]
                if x[1] not in new_extrude_tags and x[0] == 3
            ]
        ...
        if full_3D_params is not None and full_3D_params["substrate_trenching"] > 0:
            ...
            gmsh.model.occ.synchronize()
            gmsh.model.geo.synchronize()
            chip_and_trenches, chip_and_trenches_map = gmsh.model.occ.fragment(
                [(3, tag3D_chip_base)],
                new_trench_extrusions,
                removeObject=True,
                removeTool=True,
            )
            gmsh.model.occ.synchronize()
            ...
            chip_tags = [
                x[1]
                for x in chip_and_trenches_map[0]
                if x[1] not in trenches_3D and x[0] == 3
            ]
            leDielectric = gmsh.model.addPhysicalGroup(
                3, chip_tags, name="dielectric_substrate"
            )
            ...
            leDielectric = gmsh.model.addPhysicalGroup(
                3, [tag3D_chip_base], name="dielectric_substrate"
            )

        leDielectricGaps = gmsh.model.addPhysicalGroup(
            2, leDielectricGaps, name="dielectric_gaps"
        )
    ...
    def _create_gmsh_geometry_from_shapely_polygons(self, polygons):
        ...
        for m, poly in enumerate(polygons):
            poly_simplified = poly.simplify(
                1e-9
            )  # this removes points that are spaced too closely together - TODO: investigate why this is still required?
            cur_coords = poly_simplified.exterior.coords[:-1]
            if len(cur_coords) == 0 or poly_simplified.area < 1e-18:
                continue
            gmsh_exterior = self._draw_polygon_in_GMSH_from_coords(
                cur_coords
            )  # remove last coord from list because it's repeated
            if len(poly_simplified.interiors) >= 1:
                interiors = []
                for _, interior in enumerate(poly_simplified.interiors):
                    gmsh_interior = self._draw_polygon_in_GMSH_from_coords(
                        interior.coords[:-1]
                    )  # remove last coord from list because it's repeated
                    interiors.append((2, gmsh_interior))
                gmsh_surface, gmsh_surface_map = gmsh.model.occ.cut(
                    [(2, gmsh_exterior)], interiors, removeObject=True, removeTool=True
                )
    ...
    def _draw_polygon_in_GMSH_from_coords(self, coords):
        ...
        for coord in coords:
            point = gmsh.model.occ.addPoint(
                coord[0], coord[1], self._geom_processor.chip_centre[2]
            )
            points.append(point)
    ...
    def _create_chip_base(self):
        ...
        chip_base = gmsh.model.occ.addBox(
    ...
    def _draw_air_box(self, bottom_grounded):
        ...
        air_box = gmsh.model.occ.addBox(
            ...
            y_point,
            z_point,
            self.size_x + air_box_delta_x,
            self.size_y + air_box_delta_y,
            air_box_delta_z,
        )
        gmsh.model.occ.synchronize()
        self._extents = np.array(
            [
                (x_point, x_point + self.size_x + air_box_delta_x),
    ...
    def _check_port_orientation(self, vec_perp):
        ...
        assert False, (
            f"AWS Palace requires RF Lumped Ports to be aligned with the x/y axes. Here the port is pointing: {vec_perp}."
        )

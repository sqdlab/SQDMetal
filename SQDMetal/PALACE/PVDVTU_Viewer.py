...
class PVDVTU_Slice:
    ...
    def plot(self, values, cmap="viridis", equalise_histogram=False, fig=None, ax=None):
        ...
        if equalise_histogram:
            ...
            cMapNorm = matplotlib.colors.FuncNorm(
                (zinterp, zinterpInv), vmin=zMin, vmax=zMax
            )
...
class PVDVTU_Viewer:
    ...
    def get_data_slice(
        self,
        data_set_index,
        slice_plane_normal=np.array([0, 0, 1]),
        slice_plane_origin=np.array([0, 0, 0]),
    ):
        assert data_set_index < self.num_datasets, (
            f"Argument 'data_set_index' must be in: [0,{self.num_datasets - 1}]."
        )
        self._reader.set_active_time_value(self._reader.time_values[data_set_index])
        mesh = self._reader.read()
        return PVDVTU_Slice(
            mesh[0].slice(
                normal=slice_plane_normal,
                origin=slice_plane_origin,

...
class PVDVTU_Slice:
    ...
    def plot(self, values, cmap='viridis', equalise_histogram=False):
        ...
        pcm = ax.tripcolor(pts[:,0], pts[:,1], tri, values, norm=cMapNorm, cmap=cmap)
    ...
    def plot_mesh(self):
        ...
        tri = self.slc.faces.reshape((-1,4))[:, 1:]
        fig, ax = plt.subplots(1)
        ax.triplot(pts[:,0], pts[:,1], tri, linewidth=0.1, color='red')
        ax.set_xlabel('x position (m)')
        ax.set_xlabel('y position (m)')
        return fig
...
class PVDVTU_Viewer:
    ...
    def get_data_slice(self, data_set_index, slice_plane_normal=np.array([0,0,1]), slice_plane_origin=np.array([0,0,0])):
        ...
        return PVDVTU_Slice(mesh[0].slice(normal=slice_plane_normal, origin=slice_plane_origin, generate_triangles=True,  contour=False))

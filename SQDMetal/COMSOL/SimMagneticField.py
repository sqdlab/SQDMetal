...
class COMSOL_Simulation_MagneticField(COMSOL_Simulation_Base):
    ...
    def _run_premesh(self):
        ...
        self.model._model.java.component("comp1").geom("geom1").create(
            select_3D_name, "UnionSelection"
        )
        self.model._model.java.component("comp1").geom("geom1").feature(
            select_3D_name
        ...
        self.model._model.java.component("comp1").geom("geom1").feature(
            select_3D_name
        ).set("input", jtypes.JArray(jtypes.JString)(conds))
        ...
        self.jc.component("comp1").physics().create(
            self.phys_ecis, "ElectricCurrentsShell", "geom1"
        )
        self.jc.component("comp1").physics(self.phys_ecis).selection().named(
            "geom1_condPlusUBend"
        )
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").create(
            "bterm1", "BoundaryTerminal", 1
        )
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").feature(
            "bterm1"
        ).selection().named(self._sel_term_src)
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").feature(
            "bterm1"
        ).set("I0", jtypes.JFloat(1.0))
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").create(
            "bgnd1", "BoundaryGround", 1
        )
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").feature(
            "bgnd1"
        ).selection().named(self._sel_term_gnd)

        self.phys_mf = self.model._add_physics("mf")
        self.jc.component("comp1").physics().create(
            self.phys_mf, "InductionCurrents", "geom1"
        )
        self.jc.component("comp1").physics(self.phys_mf).create(
            "scu1", "SurfaceCurrent", 2
        )
        self.jc.component("comp1").physics(self.phys_mf).feature(
            "scu1"
        ).selection().named("geom1_condPlusUBend")
        self.jc.component("comp1").physics(self.phys_mf).feature("scu1").set(
            "Js0", jtypes.JArray(jtypes.JString)(["ecis.JsX", "ecis.JsY", "ecis.JsZ"])
        )
        self.jc.component("comp1").physics(self.phys_mf).create(
            "mi2", "MagneticInsulation", 2
        )
        self.jc.component("comp1").physics(self.phys_mf).feature(
            "mi2"
        ).selection().named(f"geom1_{self.current_feed_Pad}")

        self.jc.component("comp1").material("Metal").selection().named(
            "geom1_condPlusUBend"
        )
    ...
    def set_current_feed_on_CPW_on_Route(
        self, qObjName, pin_name="end", width_U=20e-6, src_gnd_gap=10e-6
    ):
        assert self.current_feed_UBend is None, (
            "Cannot have more than one current feed..."
        )

        qObj = self.model.design.components[qObjName]  # noqa: F841
        # if isinstance(qObj, LaunchpadWirebond):
        vec_ori, vec_launch, cpw_wid, cpw_gap = self._get_Route_params(
            qObjName, pin_name
        )
        # else:
        ...
        uBend = [
            ...
            vec_ori
            + vec_perp * (cpw_wid * 0.5 + cpw_gap)
            - vec_launch * src_gnd_gap
            - vec_perp * (cpw_wid + 2 * cpw_gap),
            vec_ori - vec_perp * (cpw_wid * 0.5 + cpw_gap),
            vec_ori - vec_perp * (cpw_wid * 0.5 + cpw_gap + width_U),
            vec_ori
            - vec_perp * (cpw_wid * 0.5 + cpw_gap + width_U)
            - vec_launch * (src_gnd_gap + width_U),
            vec_ori
            + vec_perp * (cpw_wid * 0.5 + cpw_gap + width_U)
            - vec_launch * (src_gnd_gap + width_U),
            vec_ori + vec_perp * (cpw_wid * 0.5 + cpw_gap + width_U),
        ...
        self.current_feed_UBend = self.model._setup_selection_boundaries(
            "FeedLnU", pol_name + "uBend"
        )
        self._sel_term_src = self.model._create_boundary_selection_sphere(
            min(cpw_wid, width_U) * 0.1, vec_ori[0], vec_ori[1], is_edge=True
        )
        ...
        self.current_feed_Pad = self.model._setup_selection_boundaries(
            "FeedLnP", pol_name + "feedPad"
        )
        vec_gnd = vec_ori - vec_launch * src_gnd_gap
        self._sel_term_gnd = self.model._create_boundary_selection_sphere(
            min(cpw_wid, width_U) * 0.1, vec_gnd[0], vec_gnd[1], is_edge=True
        )
    ...
    def set_current_feed_point_point_verticalU(self, x1, y1, x2, y2, uWidth):
        ...
        self.jc.component("comp1").geom("geom1").feature(wpName).set(
            "unite", jtypes.JBoolean(True)
        )
        self.jc.component("comp1").geom("geom1").feature(wpName).set(
            "planetype", "normalvector"
        )
        self.jc.component("comp1").geom("geom1").feature(wpName).set(
            "normalvector", jtypes.JArray(jtypes.JDouble)([y1 - y2, x2 - x1, 0.0])
        )
        self.jc.component("comp1").geom("geom1").feature(wpName).set(
            "normalcoord",
            jtypes.JArray(jtypes.JDouble)([(x1 + x2) * 0.5, (y1 + y2) * 0.5, 0.0]),
        )
        self.jc.component("comp1").geom("geom1").feature(wpName).set(
            "rot", jtypes.JDouble(90)
        )
        ...
        self.jc.component("comp1").geom("geom1").feature(wpName).geom().create(
            polyUname, "Polygon"
        )
        self.jc.component("comp1").geom("geom1").feature(wpName).geom().feature(
            polyUname
        ...
        self.jc.component("comp1").geom("geom1").feature(
            wpName
        ).geom().selection().create(selPolyNameWP, "CumulativeSelection")
        self.jc.component("comp1").geom("geom1").feature(wpName).geom().feature(
            polyUname
        ).set("contributeto", selPolyNameWP)
        #
        select_3D_name = "selCurrentFeedVerticalU"
        self.jc.component("comp1").geom("geom1").create(
            select_3D_name, "UnionSelection"
        )
        self.jc.component("comp1").geom("geom1").feature(select_3D_name).label(
            select_3D_name
        )
        self.jc.component("comp1").geom("geom1").feature(select_3D_name).set(
            "entitydim", jtypes.JInt(2)
        )
        self.jc.component("comp1").geom("geom1").feature(select_3D_name).set(
            "input", f"{wpName}_{selPolyNameWP}"
        )

        self._sel_term_src = self.model._create_boundary_selection_sphere(
            uWidth * 0.1, x1, y1, is_edge=True
        )
        self._sel_term_gnd = self.model._create_boundary_selection_sphere(
            uWidth * 0.1, x2, y2, is_edge=True
        )
        self.current_feed_UBend = None
    ...
    def set_Bfield_integration_area(self, coords):
        ...
        self._bfield_int = self.model._setup_selection_boundaries(
            "BFieldIntegration", "bfieldIntArea"
        )
    ...
    def run(self):
        ...
        self.jc.result().numerical("int1").selection().named(
            "geom1_" + self._bfield_int
        )
        self.jc.result().numerical("int1").set(
            "expr", jtypes.JArray(jtypes.JString)(["mf.Bz"])
        )
        self.jc.result().numerical("int1").set(
            "descr",
            jtypes.JArray(jtypes.JString)(["Magnetic flux density, z-component"]),
        )
        self.jc.result().numerical("int1").set(
            "unit", jtypes.JArray(jtypes.JString)(["Wb"])
        )
        #

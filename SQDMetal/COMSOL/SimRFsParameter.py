...
class COMSOL_Simulation_RFsParameters(COMSOL_Simulation_Base):
    def __init__(
        ...
        model,
        adaptive="None",
        modal_min_freq_num_eigs=(1e9, 7),
        relative_tolerance=0.01,
        include_loss_tangents=False,
    ):
        self.model = model
        ...
        self._ports = []
        self.lumped_elems = []
        assert adaptive == "None" or adaptive == "Single" or adaptive == "Multiple", (
            "Adaptive Simulation must be None, Single or Multiple"
        )
    ...
    def _prepare_simulation(self):
        ...
        if self.adaptive == "None":
            ...
            self.jc.study(self._study).feature(self._sub_study).set(
                "savesolsref", jtypes.JBoolean(False)
            )
            ...
            self.jc.study(self._study).feature(self._sub_study[0]).activate(
                "emw", jtypes.JBoolean(True)
            )
            ...
            self.jc.study(self._study).feature(self._sub_study[1]).set(
                "notsolnum", "auto"
            )
            self.jc.study(self._study).feature(self._sub_study[1]).activate(
                "emw", jtypes.JBoolean(True)
            )
            self.jc.study(self._study).feature(self._sub_study[0]).set(
                "shift", f"{self.modal_min_freq_num_eigs[0] * 1e-9}[GHz]"
            )
            self.jc.study(self._study).feature(self._sub_study[0]).set(
                "neigsactive", jtypes.JBoolean(True)
            )
            self.jc.study(self._study).feature(self._sub_study[0]).set(
                "neigs", f"{self.modal_min_freq_num_eigs[1]}"
            )
            # Only store the port solutions for Multi-Modal...
            lst_ports = ["geom1_" + cur_port["polys"][0] for cur_port in self._ports]
            self.jc.study(self._study).feature(self._sub_study[1]).set(
                "usestoresel", "selection"
            )
            self.jc.study(self._study).feature(self._sub_study[1]).set(
                "storesel", jtypes.JArray(jtypes.JString)(lst_ports)
            )
        ...
        self.jc.result().numerical(int_name).set(
            "expr", jtypes.JArray(jtypes.JString)(["emw.Ez"])
        )
        self.jc.result().numerical(int_name).set(
            "descr", jtypes.JArray(jtypes.JString)(["Electric field, z-component"])
        )
        #
        int_name = "intDielectric"
        self.jc.result().numerical().create(int_name, "IntSurface")
        self.jc.result().numerical(int_name).set("intvolume", jtypes.JBoolean(True))
        self.jc.result().numerical(int_name).selection().named(
            "geom1_dielectricSurface"
        )
        self.jc.result().numerical(int_name).set(
            "expr", jtypes.JArray(jtypes.JString)(["emw.Ez"])
        )
    ...
    def _temporary_freq_domain_AutoSequence(self):
        ...
        self.jc.sol(self._soln).feature("s1").feature("p1").set(
            "pname", jtypes.JArray(jtypes.JString)(["freq"])
        )
        self.jc.sol(self._soln).feature("s1").feature("p1").set(
            "plistarr", jtypes.JArray(jtypes.JString)(["1.0, 2.0, 3.0"])
        )
        self.jc.sol(self._soln).feature("s1").feature("p1").set(
            "punit", jtypes.JArray(jtypes.JString)(["GHz"])
        )
        self.jc.sol(self._soln).feature("s1").feature("p1").set(
        ...
        self.jc.sol(self._soln).feature("s1").feature("p1").set(
            "probes", jtypes.JArray(jtypes.JString)([])
        )
        ...
        self.jc.sol(self._soln).feature("s1").feature("aDef").set(
            "complexfun", jtypes.JBoolean(True)
        )
        ...
        self.jc.sol(self._soln).feature("s1").feature("i1").label(
            "Suggested Iterative Solver (emw)"
        )
        self.jc.sol(self._soln).feature("s1").feature("i1").create("mg1", "Multigrid")
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").set(
            "iter", "1"
        )
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "pr"
        ).create("sv1", "SORVector")
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "pr"
        ).feature("sv1").set("prefun", "sorvec")
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "pr"
        ).feature("sv1").set("iter", jtypes.JInt(2))
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "pr"
        ).feature("sv1").set("relax", jtypes.JInt(1))
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "pr"
        ).feature("sv1").set("sorvecdof", jtypes.JArray(jtypes.JString)(["comp1_E"]))
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "po"
        ).create("sv1", "SORVector")
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "po"
        ).feature("sv1").set("prefun", "soruvec")
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
            "po"
        ).feature("sv1").set("iter", jtypes.JInt(2))
        self.jc.sol(self._soln).feature("s1").feature("i1").feature("mg1").feature(
    ...
    def _run_premesh(self):
        ...
        self.jc.component("comp1").physics().create(
            self.phys_emw, "ElectromagneticWaves", "geom1"
        )
        # Create any lumped element ports
        for cur_elem_id, cur_lelem in enumerate(self.lumped_elems):
            elem_name = "lelem" + str(cur_elem_id)
            self.jc.component("comp1").physics(self.phys_emw).create(
                elem_name, "LumpedElement", 2
            )
            elem_bnds = self.model._get_selection_boundaries(cur_lelem[0])
            self.jc.component("comp1").physics(self.phys_emw).feature(
                elem_name
            ).selection().set(jtypes.JArray(jtypes.JInt)(elem_bnds))
            self.jc.component("comp1").physics(self.phys_emw).feature(elem_name).set(
                "LumpedElementType", cur_lelem[2]
            )
            if cur_lelem[2] == "Inductor":
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    elem_name
                ).set("Lelement", jtypes.JDouble(cur_lelem[1]))
            elif cur_lelem[2] == "LCparallel":
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    elem_name
                ).set("Lelement", jtypes.JDouble(cur_lelem[1][0]))
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    elem_name
                ).set("Celement", jtypes.JDouble(cur_lelem[1][1]))
                ...
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    elem_name
                ).set("Relement", jtypes.JDouble(cur_lelem[1][2]))

        if self.model._include_loss_tangents:
            self.jc.component("comp1").physics("emw").feature("wee1").set(
                "DisplacementFieldModel", "LossTangent"
            )

        # Note that PEC1 is the default exterior boundary condition
        if not self._discard_PEC_BC:
            self.jc.component("comp1").physics(self.phys_emw).create(
                "pec2", "PerfectElectricConductor", 2
            )
            self.jc.component("comp1").physics(self.phys_emw).feature(
                "pec2"
            ).selection().named("geom1_condAll")
        # Create the excitation ports
        self.port_names = []
        for cur_port_id, cur_port in enumerate(self._ports):
            port_name = "lport" + str(cur_port_id)
            self.jc.component("comp1").physics(self.phys_emw).create(
                port_name, "LumpedPort", 2
            )
            if cur_port["type"] == "CPW":
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
                ).set("PortType", "MultiElementUniform")
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
                ).selection().named("geom1_" + cur_port["polys"][0])
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
                ).feature("ue1").selection().named("geom1_" + cur_port["polys"][1])
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
                ).feature("ue2").selection().named("geom1_" + cur_port["polys"][2])
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
                ).feature("ue1").set(
                    "ahUniformElement",
                    jtypes.JArray(jtypes.JDouble)(
                        [cur_port["vec_perp"][0], cur_port["vec_perp"][1], 0.0]
                    ),
                )
                self.jc.component("comp1").physics(self.phys_emw).feature(
                    port_name
        ...
        for m, cur_current_src in enumerate(self._current_sources):
            ...
            if cur_current_src[0] == "region":
                ...
                self.jc.component("comp1").geom("geom1").feature().create(
                    cur_extrusion, "Extrude"
                )
                self.jc.component("comp1").geom("geom1").feature(cur_extrusion).set(
                    "extrudefrom", "faces"
                )
                self.jc.component("comp1").geom("geom1").feature(
                    cur_extrusion
                ).selection("inputface").named("condAll")
                self.jc.component("comp1").geom("geom1").feature(
                    cur_extrusion
                ).setIndex("distance", str(extrusion_depth), 0)
        ...
        for m, cur_dipole in enumerate(self._electric_point_dipoles):
            ...
            edp_name = f"edp{m}"
            self.jc.component("comp1").physics(self.phys_emw).create(
                edp_name, "ElectricPointDipole", 0
            )
            self.jc.component("comp1").physics(self.phys_emw).feature(
                edp_name
            ).selection().named(
                f"geom1_{sel_name}_pnt"
            )  # Why does it require this _pnt?
            self.jc.component("comp1").physics(self.phys_emw).feature(edp_name).set(
                "enpI", jtypes.JArray(jtypes.JDouble)(dipole_moment_vecdir.tolist())
            )
            self.jc.component("comp1").physics(self.phys_emw).feature(edp_name).set(
                "normpI", jtypes.JFloat(dipole_moment_p)
            )
    ...
    def create_port_2_conds_by_position(self, pos1, pos2, rect_width=20e-6):
        ...
        select_3D_name = self.model._setup_selection_boundaries(
            len(self._ports), f"port{len(self._ports)}", "port"
        )
    ...
    def create_port_CPW_on_Launcher(self, qObjName, len_launch=20e-6):
        ...
        launchesA, launchesB, vec_perp = QUtilities.get_RFport_CPW_coords_Launcher(
            self.model.design, qObjName, len_launch
        )

        cur_port = {"type": "CPW"}

        sel_x, sel_y, sel_r = self.model._create_poly(pol_name + "a", launchesA)
        select_3D_nameA = self.model._setup_selection_boundaries(
            len(self._ports), pol_name + "a", "porta"
        )

        sel_x, sel_y, sel_r = self.model._create_poly(pol_name + "b", launchesB)
        select_3D_nameB = self.model._setup_selection_boundaries(
            len(self._ports), pol_name + "b", "portb"
        )

        sel_all_name = f"uniselPort{len(self._ports)}"
        self.jc.component("comp1").geom("geom1").create(sel_all_name, "UnionSelection")
        self.jc.component("comp1").geom("geom1").feature(sel_all_name).label(
            sel_all_name
        )
        self.jc.component("comp1").geom("geom1").feature(sel_all_name).set(
            "entitydim", jtypes.JInt(2)
        )
        self.jc.component("comp1").geom("geom1").feature(sel_all_name).set(
            "input", jtypes.JArray(jtypes.JString)([select_3D_nameA, select_3D_nameB])
        )
    ...
    def create_port_CPW_on_Route(self, qObjName, pin_name="end", len_launch=20e-6):
        ...
        launchesA, launchesB, vec_perp = QUtilities.get_RFport_CPW_coords_Route(
            self.model.design, qObjName, pin_name, len_launch
        )
    ...
    def create_RFport_CPW_groundU_Launcher_inplane(
        ...
        Uclip = QUtilities.get_RFport_CPW_groundU_Launcher_inplane(
            self.model.design, qObjName, thickness_side, thickness_back, separation_gap
        )
    ...
    def create_RFport_CPW_groundU_Route_inplane(
        ...
        Uclip = QUtilities.get_RFport_CPW_groundU_Route_inplane(
            self.model.design,
            route_name,
            pin_name,
            thickness_side,
    ...
    def create_lumped_inductance(self, qObj1, pin1, qObj2, pin2, width, value):
        ...
        sel_x, sel_y, sel_r = self.model._create_poly(
            f"lumpedPort{len(self.lumped_elems)}",
            np.array(
                [
                    pos1 + v_perp,
                    pos1 - v_perp,
        ...
        self.lumped_elems.append(
            [
                self.model._create_boundary_selection_sphere(sel_r, sel_x, sel_y),
                value,
                "Inductor",
            ]
        )
    ...
    def create_port_JosephsonJunction(self, qObjName, **kwargs):
        ...
        sel_x, sel_y, sel_r = self.model._create_poly(
            f"lumpedPort{len(self.lumped_elems)}", np.array(portCoords)
        )
        if C_J > 0:
            if R_J > 0:
                self.lumped_elems.append(
                    [
                        self.model._create_boundary_selection_sphere(
                            sel_r, sel_x, sel_y
                        ),
                        (L_ind, C_J, R_J),
                        "RLCparallel",
    ...
    def set_freq_range(
        self, freq_start, freq_end, num_points, use_previous_solns=False
    ):
        """
        ...
        """
        if num_points > 1:
            str_freqs = "range({0}[GHz],({1}[GHz]-({0}[GHz]))/{2},{1}[GHz])".format(
                freq_start * 1e-9, freq_end * 1e-9, num_points - 1
            )
        else:
            str_freqs = "{0}[GHz]*1^range(1,1)".format(
                freq_start * 1e-9,
            )
    ...
    def add_surface_current_source_region(
        self, region="", extrusion_depth=1e-6, J_z=1.0
    ):
        assert self.adaptive == "None", (
            "Cannot use eigenmode-based simulations when setting current sources."
        )
    ...
    def add_electric_point_dipole(self, vec_pos, dipole_moment_p, dipole_moment_vecdir):
        ...
        self.jc.component("comp1").geom("geom1").feature(pt_name).setIndex(
            "p", str(vec_pos[0]), 0
        )
        self.jc.component("comp1").geom("geom1").feature(pt_name).setIndex(
            "p", str(vec_pos[1]), 1
        ...
        self.jc.component("comp1").geom("geom1").selection().create(
            sel_name, "CumulativeSelection"
        )
        self.jc.component("comp1").geom("geom1").selection(sel_name).label(pt_name)
        self.jc.component("comp1").geom("geom1").feature(pt_name).set(
            "contributeto", sel_name
        )
        #
        dipole_moment_vecdir = np.array(dipole_moment_vecdir)
        self._electric_point_dipoles.append(
            (
                sel_name,
                dipole_moment_p,
                dipole_moment_vecdir / np.linalg.norm(dipole_moment_vecdir),
            )
        )
    ...
    def run(self, recompute=True, is_dB=False):
        ...
        for cur_ind in range(0, len(self._ports)):
            if is_dB:
                ...
                # For some reason the returned arrays are rows of the same data apparently repeated across the columns...
                self.jc.result().numerical("ev1").set(
                    "expr", f"real(emw.S{cur_ind + 1}1)"
                )
                cur_sValsR = self.jc.result().numerical("ev1").getData()
                self.jc.result().numerical("ev1").set(
                    "expr", f"imag(emw.S{cur_ind + 1}1)"
                )
                cur_sValsI = self.jc.result().numerical("ev1").getData()
                #
                cur_sVals = (
                    np.array(cur_sValsR[0])[:, 0] + 1j * np.array(cur_sValsI[0])[:, 0]
                )
            s1Remains += [cur_sVals]
    ...
    def eval_fields_over_mesh(self):
        x, y, z, Ex, Ey, Ez, Bx, By, Bz, Dx, Dy, Dz, Hx, Hy, Hz = (
            self.model._model.evaluate(
                [
                    "x",
                    "y",
                    "z",
    ...
    def eval_field_at_pts(self, expr, coords: np.ndarray, freq_index=1):
        ...
        self.jc.result().numerical("pev1").setIndex(
            "looplevelinput", "manualindices", 0
        )
        self.jc.result().numerical("pev1").setIndex(
            "looplevelindices", jtypes.JInt(freq_index), 0
        )
    ...
    def run_only_eigenfrequencies(self, return_dofs=False):
        ...
        self.jc.result().numerical("gev1").set(
            "expr", jtypes.JArray(jtypes.JString)(["numberofdofs"])
        )
        ...
        freqs = [
            1e9 * np.complex128(str(y).replace("i", "j"))
            for y in [x[0] for x in self.jc.result().table("tbl1").getTableData(True)]
        ]
        dofs = int(
            np.array(self.jc.result().numerical("gev1").computeResult())[0, 0, 0]
        )
    ...
    def find_peak(
        ...
        sol = scipy.optimize.minimize(
            lambda x: self._cost_func(x, s_param_index, find_max),
            [initial_guess],
            method="Nelder-Mead",
            bounds=(scipy.optimize.Bounds(search_bnd_min_freq, search_bnd_max_freq)),
            options={"xatol": freq_accuracy},
        )

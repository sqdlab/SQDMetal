...
class COMSOL_Simulation_CapMats(COMSOL_Simulation_Base):
    ...
    def _run_premesh(self):
        ...
        self.jc.component("comp1").physics().create(
            self.phys_es, "Electrostatics", "geom1"
        )
        self.jc.component("comp1").physics(self.phys_es).prop("PortSweepSettings").set(
            "useSweep", jtypes.JBoolean(True)
        )
        self.jc.component("comp1").physics(self.phys_es).prop("PortSweepSettings").set(
            "PortParamName", "PortName"
        )
        ...
        for cur_term in range(len(self.model._conds)):
            ...
            self.jc.component("comp1").physics(self.phys_es).create(
                term_name, "Terminal", 2
            )
            self.jc.component("comp1").physics(self.phys_es).feature(
                term_name
            ).selection().named("".join(self.model._conds[cur_term]))
            self.jc.component("comp1").physics(self.phys_es).feature(term_name).set(
                "TerminalType", "Voltage"
            )
            self.jc.component("comp1").physics(self.phys_es).feature(term_name).set(
                "TerminalName", jtypes.JInt(cur_term + 1)
            )
        ...
        self.jc.component("comp1").physics(self.phys_es).feature(
            "gnd1"
        ).selection().named(self.model._sel_ext_boundaries)

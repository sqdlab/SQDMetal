...
class COMSOL_Model:
    ...
    def initialize_model(self, design, simulations, **kwargs):
        ...
        if (
            ...
            self.chip_len = QUtilities.parse_value_length(
                design.chips["main"]["size"]["size_x"]
            )
            self.chip_wid = QUtilities.parse_value_length(
                design.chips["main"]["size"]["size_y"]
            )
            self.chip_centre = [
                QUtilities.parse_value_length(design.chips["main"]["size"][x])
        ...
        self.chip_thickness = np.abs(
            QUtilities.parse_value_length(design.chips["main"]["size"]["size_z"])
        )
        ...
        self._create_block_centre(
            ...
            self.chip_centre[0],
            self.chip_centre[1],
            -self.chip_thickness * 0.5,
        )
        if self.bottom_grounded:
            self._create_block_centre(
                "blk_boundary",
                self.chip_len + 2 * self.pad_x,
                self.chip_wid + 2 * self.pad_y,
                self.chip_thickness + self.pad_z,
                self.chip_centre[0],
                self.chip_centre[1],
                (self.pad_z - self.chip_thickness) * 0.5,
            )
        else:
            self._create_block_centre(
                "blk_boundary",
                self.chip_len + 2 * self.pad_x,
                self.chip_wid + 2 * self.pad_y,
                self.chip_thickness + 2 * self.pad_z,
                self.chip_centre[0],
                self.chip_centre[1],
                -self.chip_thickness * 0.5,
            )
        ...
        self._model.java.component("comp1").geom("geom1").feature("blk_boundary").set(
            "selresultshow", "bnd"
        )
        self._model.java.component("comp1").geom("geom1").selection().create(
            "cBndOuter", "CumulativeSelection"
        )
        self._model.java.component("comp1").geom("geom1").selection("cBndOuter").label(
            "bound_outer"
        )
        self._model.java.component("comp1").geom("geom1").feature("blk_boundary").set(
            "contributeto", "cBndOuter"
        )
        self._sel_ext_boundaries = "geom1_cBndOuter_bnd"

        # Create workplane and subsequent metallic geometry...
        self._model.java.component("comp1").geom("geom1").feature().create(
            "wp1", "WorkPlane"
        )
        self._model.java.component("comp1").geom("geom1").feature("wp1").set(
            "unite", jtypes.JBoolean(True)
        )  # Unite all objects...
    ...
    def add_metallic(self, layer_id, **kwargs):
        ...
        metal_polys_all, metal_sel_ids = QUtilities.get_metals_in_layer(
            self.design, layer_id, **kwargs
        )
        ...
        for m in range(len(metal_polys_all)):
            ...
            if len(poly_interiors) > 0:
                ...
                for ind, cur_int in enumerate(poly_interiors):
                    ...
                    sel_x, sel_y, sel_r = self._create_poly(
                        pol_name_int, cur_poly_int[:-1]
                    )
                # Subtract interiors from the polygon
                diff_name = f"difInt{self._num_polys + m}"
                self._model.java.component("comp1").geom("geom1").feature(
                    "wp1"
                ).geom().create(diff_name, "Difference")
                self._model.java.component("comp1").geom("geom1").feature(
                    "wp1"
                ).geom().feature(diff_name).selection("input").set(pol_name)
                self._model.java.component("comp1").geom("geom1").feature(
                    "wp1"
                ).geom().feature(diff_name).selection("input2").set(*pol_name_ints)
        ...
        for cur_id in metal_sel_obj_names:
            ...
            select_3D_name = self._setup_selection_boundaries(
                cur_sel_index, metal_sel_obj_names[cur_id]
            )
            self._conds.append(
                ("geom1_", select_3D_name)
            )  # Prefix for accessing out of the geometry node...
            # Could optimise this line...
            self._cond_polys.append(
                (
                    shapely.unary_union(
                        [
                            metal_polys_all[m]
                            for m in range(len(metal_polys_all))
                            if metal_sel_ids[m] == cur_id
                        ]
                    ),
                    cur_sel_index,
                )
    ...
    def fuse_all_metals(self, fuse_threshold=1e-12):
        ...
        while len(inds_left) > 0:
            ...
            while True:
                ...
                for cur_ind in other_poly_inds:
                    ...
                    cur_check = ShapelyEx.fuse_polygons_threshold(
                        [cur_blobotron, cur_check_poly], fuse_threshold
                    )
                    if COMSOL_Model.get_poly_cardinality(
                        cur_check
                    ) < COMSOL_Model.get_poly_cardinality(
                        cur_blobotron
                    ) + COMSOL_Model.get_poly_cardinality(cur_check_poly):
        ...
        for cur_ind in new_ind_groups:
            ...
            self._model.java.component("comp1").geom("geom1").create(
                select_3D_name, "UnionSelection"
            )
            self._model.java.component("comp1").geom("geom1").feature(
                select_3D_name
            ).label(select_3D_name)
            self._model.java.component("comp1").geom("geom1").feature(
                select_3D_name
            ).set("entitydim", jtypes.JInt(2))
            self._model.java.component("comp1").geom("geom1").feature(
                select_3D_name
            ).set(
                "input",
                jtypes.JArray(jtypes.JString)(
                    [self._conds[x][1] for x in new_ind_groups[cur_ind]]
                ),
            )
    ...
    def reorder_conds_by_comps(self, comp_list):
        ...
        for cur_comp in comp_list:
            ...
            polys = gsdf.loc[
                (gsdf["component"] == self.design.components[cur_comp].id)
                & (~gsdf["subtract"])
            ]["geometry"].to_list()
            if len(polys) > 1:
                poly = shapely.unary_union(polys)
                assert isinstance(poly, shapely.Polygon), (
                    f"Component '{cur_comp}' is not a contiguous polygonal object."
                )
            else:
                poly = polys[0]
            poly = shapely.affinity.scale(
                poly, xfact=unit_conv, yfact=unit_conv, origin=(0, 0)
            )
            poly = pvdSh.get_all_shadows(
                poly, self.design.components[cur_comp].options.layer, "merge"
            )
        ...
        residual_conds = [
            (self._conds[m], self._cond_polys[m][0]) for m in range(len(self._conds))
        ]
    ...
    def _simplify_geom(self, cur_poly, threshold):
        if threshold > 0:
            ...
            for m in range(1, len(cur_poly)):
                if (
                    np.sqrt(
                        (cur_poly[m][0] - cur_poly[m - 1][0]) ** 2
                        + (cur_poly[m][1] - cur_poly[m - 1][1]) ** 2
                    )
                    > threshold
                ):
                    fin_poly += [cur_poly[m]]
    ...
    def _setup_selection_boundaries(self, index, select_obj_name, sel_3D_prefix="cond"):
        ...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().create(
            select_name, "ExplicitSelection"
        )
        for cur_sel_name in select_obj_name:
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(select_name).selection("selection").set(cur_sel_name, 1)
        select_3D_name = f"{sel_3D_prefix}{index}"
        self._model.java.component("comp1").geom("geom1").create(
            select_3D_name, "UnionSelection"
        )
        self._model.java.component("comp1").geom("geom1").feature(select_3D_name).label(
            select_3D_name
        )
        self._model.java.component("comp1").geom("geom1").feature(select_3D_name).set(
            "entitydim", jtypes.JInt(2)
        )
        self._model.java.component("comp1").geom("geom1").feature(select_3D_name).set(
            "input", f"wp1_{select_name}"
        )
        ...
        if len(space_polys) > 0:
            ...
            for ind, cur_cut in enumerate(space_polys):
                ...
                if len(poly_interiors) > 0:
                    ...
                    self._model.java.component("comp1").geom("geom1").feature(
                        "wp1"
                    ).geom().feature(diff_name).selection("input").set(pol_ext_name)
                    self._model.java.component("comp1").geom("geom1").feature(
                        "wp1"
                    ).geom().feature(diff_name).selection("input2").set(*pol_name_ints)
                else:
                    sel_x, sel_y, sel_r = self._create_poly(
                        pol_name_cut, cur_poly_cut[:-1]
                    )
            ...
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(diff_name).selection("input2").set(*pol_name_cuts)
            # Setup the resulting selection...
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(diff_name).set("selresult", jtypes.JBoolean(True))
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(diff_name).set("selresultshow", "bnd")
            select_2D_name = "cselGND"
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().selection().create(select_2D_name, "CumulativeSelection")
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().selection(select_2D_name).label("Cumulative Selection Ground")
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(diff_name).set("contributeto", select_2D_name)
            ...
            self._model.java.component("comp1").geom("geom1").feature(
                select_3D_name
            ).set("input", jtypes.JArray(jtypes.JString)(["wp1_" + select_2D_name]))
            cur_select_index = len(self._conds)
        else:
            select_obj_name = pol_name
            cur_select_index = len(self._conds)
            select_3D_name = self._setup_selection_boundaries(
                cur_select_index, select_obj_name
            )
        self._conds.append(
            ("geom1_", select_3D_name)
        )  # Prefix for accessing out of the geometry node...
    ...
    def fine_mesh_in_rectangle(
        self, x1, y1, x2, y2, minElementSize=1e-7, maxElementSize=5e-6
    ):
        assert x2 > x1 and y2 > y1, (
            "Ensure (x1,y1) is the bottom-left corner while (x2,y2) is the top-right corner of the rectangle."
        )
        ...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            rect_name
        ).set("size", jtypes.JArray(jtypes.JDouble)([x2 - x1, y2 - y1]))
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            rect_name
        ).set("pos", jtypes.JArray(jtypes.JDouble)([x1, y1]))
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().run(
            rect_name
        )  # Makes selection easier...
        #
        sel_mesh_name = f"selMesh{ind}"
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().create(
            sel_mesh_name, "ExplicitSelection"
        )
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            sel_mesh_name
        ).selection("selection").set(rect_name, 1)
        #
        ...
        self._fine_mesh += [
            {
                "type": "all",
                "poly": leLine,
                "sel_rect": "wp1_" + sel_mesh_name,
                "minElem": minElementSize,
                "maxElem": maxElementSize,
            }
        ]

    def fine_mesh_conductors_in_rectangle(
        self, x1, y1, x2, y2, minElementSize=1e-7, maxElementSize=5e-6
    ):
        assert x2 > x1 and y2 > y1, (
            "Ensure (x1,y1) is the bottom-left corner while (x2,y2) is the top-right corner of the rectangle."
    ...
    def fine_mesh_in_poly(
        ...
        leCoords = np.array(
            poly.buffer(buffer_size, join_style=2, cap_style=3).exterior.coords[:]
        )
        if qmUnits:
        ...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            sel_mesh_name
        ).selection("selection").set(pol_name, 1)
        #
        self._fine_mesh += [
            {
                "type": "all",
                "poly": shapely.LineString(leCoords),
                "sel_rect": "wp1_" + sel_mesh_name,
                "minElem": minElementSize,
                "maxElem": maxElementSize,
            }
    ...
    def fine_mesh_around_comp_boundaries(
        self, list_comp_names, minElementSize=1e-7, maxElementSize=5e-6, **kwargs
    ):
        kwargs["restrict_rect"] = self.restrict_rect
        kwargs["resolution"] = self._resolution
        list_polys = QUtilities.get_perimetric_polygons(
            self.design, list_comp_names, **kwargs
        )
        self.fine_mesh_in_polys(
            list_polys,
            minElementSize,
            maxElementSize,
            qmUnits=False,
    ...
    def fine_mesh_in_polys(
        ...
        for pol_name in pol_names:
            self._model.java.component("comp1").geom("geom1").feature(
                "wp1"
            ).geom().feature(sel_mesh_name).selection("selection").set(pol_name, 1)
        #
        self._fine_mesh += [
            {
                "type": "all",
                "poly": shapely.MultiLineString(leLines),
                "sel_rect": "wp1_" + sel_mesh_name,
                "minElem": minElementSize,
                "maxElem": maxElementSize,
            }
    ...
    def _create_block_corner(self, name, size_x, size_y, size_z, pos_x, pos_y, pos_z):
        ...
        self._model.java.component("comp1").geom("geom1").feature(name).set(
            "base", "corner"
        )
        self._model.java.component("comp1").geom("geom1").feature(name).set(
            "size", jtypes.JArray(jtypes.JDouble)([size_x, size_y, size_z])
        )
        self._model.java.component("comp1").geom("geom1").feature(name).set(
            "pos", jtypes.JArray(jtypes.JDouble)([pos_x, pos_y, pos_z])
        )
        self._model.java.component("comp1").geom("geom1").feature(name).set(
            "selresult", "on"
        )  # To generate automatic selections...
    ...
    def _create_poly(self, name, poly_coords):
        ...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            name
        ).set("selresult", "on")  # To generate automatic selections...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            name
        ).set("selresultshow", "bnd")
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            name
        ).set("source", "table")
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            ...
            "table", jtypes.JArray(jtypes.JDouble, 2)(poly_coords)
        )  # This is much faster...
    ...
    def _find_point_in_polygon(self, poly_coords):
        ...
        vec1 = np.array(
            [
                poly_coords[min_ind2][0] - poly_coords[min_ind][0],
                poly_coords[min_ind2][1] - poly_coords[min_ind][1],
            ]
        )
        vec2 = np.array(
            [
                poly_coords[min_ind - 1][0] - poly_coords[min_ind][0],
                poly_coords[min_ind - 1][1] - poly_coords[min_ind][1],
            ]
        )
        ...
        epsilon_mov = 10e-9
        rad = (
            np.sqrt(min(1 - np.dot(vec1, vec3) ** 2, 1 - np.dot(vec2, vec3) ** 2))
            * 0.9
            * epsilon_mov
        )
        vec3 *= epsilon_mov
        return (
            vec3[0] + poly_coords[min_ind][0],
            vec3[1] + poly_coords[min_ind][1],
            rad,
        )

    def _create_boundary_selection_sphere(
        self, radius, pos_x, pos_y, pos_z=0.0, is_edge=False
    ):
        """
        ...
        self._model.java.selection(sel_name).set(
            "entitydim", "2" if not is_edge else "1"
        )
    ...
    def _create_material(
        self, name, rel_permit, rel_permea, conductivity, loss_tangent=0
    ):
        """
        ...
        if loss_tangent > 0:
            self._model.java.component("comp1").material(name).propertyGroup("def").set(
                "relpermittivity", f"{rel_permit}*(1-{loss_tangent}i)"
            )
        else:
            self._model.java.component("comp1").material(name).propertyGroup("def").set(
                "relpermittivity", jtypes.JDouble(rel_permit)
            )
        self._model.java.component("comp1").material(name).propertyGroup("def").set(
            "relpermeability", jtypes.JDouble(rel_permea)
        )
        self._model.java.component("comp1").material(name).propertyGroup("def").set(
            "electricconductivity", jtypes.JDouble(conductivity)
        )
    ...
    def build_geom_mater_elec_mesh(
        self,
        mesh_structure="Normal",
        skip_meshing=False,
        substrate_material=None,
        substrate_permittivity=11.45,
        **kwargs,
    ):  # mesh_structure can be 'Fine'
        """
        ...
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().create(
            sel_surface_name, "ExplicitSelection"
        )
        self._model.java.component("comp1").geom("geom1").feature("wp1").geom().feature(
            sel_surface_name
        ).selection("selection").set(pol_name, 1)
        ...
        self._model.java.component("comp1").geom("geom1").feature(select_3D_name).set(
            "input", jtypes.JArray(jtypes.JString)([x[1] for x in self._conds])
        )
        self._model.java.component("comp1").geom("geom1").run("condAll")
        #
        diff_sel_name = "dielectricSurface"
        self._model.java.component("comp1").geom("geom1").create(
            diff_sel_name, "DifferenceSelection"
        )
        self._model.java.component("comp1").geom("geom1").feature(diff_sel_name).set(
            "entitydim", jtypes.JInt(2)
        )
        self._model.java.component("comp1").geom("geom1").feature(diff_sel_name).set(
            "add", jtypes.JArray(jtypes.JString)([f"wp1_{sel_surface_name}"])
        )
        self._model.java.component("comp1").geom("geom1").feature(diff_sel_name).set(
            "subtract", jtypes.JArray(jtypes.JString)(["condAll"])
        )
        ...
        self._create_material(
            "Substrate", Er, 1.0, 0.0, loss_tangent
        )  # Don't use 4.35e-4 for conductivity as it can cause issues with convergence in inductance simulations...
        self._model.java.component("comp1").material("Substrate").selection().set(
            self._model.java.selection("geom1_blk_chip_dom").entities(3)
        )
        ...
        self._model.java.component("comp1").material("Metal").selection().geom(
            "geom1", 2
        )
        self._model.java.component("comp1").material("Metal").selection().named(
            "geom1_condAll"
        )
        ...
        for m, fmData in enumerate(self._fine_mesh):
            ...
            self._model.java.component("comp1").geom("geom1").create(
                sel_mesh_name, "IntersectionSelection"
            )
            self._model.java.component("comp1").geom("geom1").feature(
                sel_mesh_name
            ).set("entitydim", jtypes.JInt(2))
            ...
            self._model.java.component("comp1").geom("geom1").feature(
                sel_mesh_name
            ).set("input", jtypes.JArray(jtypes.JString)(sel_list))
            self._model.java.component("comp1").geom("geom1").feature(
                sel_mesh_name
            ).label(f"selFineMesh{m}")
            self._model.java.component("comp1").geom("geom1").run(sel_mesh_name)

            mesh_name = "ftri" + str(m)
            self._model.java.component("comp1").mesh("mesh1").create(
                mesh_name, "FreeTri"
            )
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).selection().named("geom1_" + sel_mesh_name)
            self._model.java.component("comp1").mesh("mesh1").feature(mesh_name).create(
                "size1", "Size"
            )
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
                ...
                "hauto", jtypes.JInt(1)
            )  # Setting to extremely fine and then changing it - does this even do anything?
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).feature("size1").set("custom", jtypes.JBoolean(True))
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).feature("size1").set("hmaxactive", jtypes.JBoolean(True))
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).feature("size1").set("hminactive", jtypes.JBoolean(True))
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).feature("size1").set("hmin", jtypes.JDouble(fmData["minElem"]))
            self._model.java.component("comp1").mesh("mesh1").feature(
                mesh_name
            ).feature("size1").set("hmax", jtypes.JDouble(fmData["maxElem"]))
        ...
        self._model.java.component("comp1").mesh("mesh1").feature("ftet10").create(
            "size1", "Size"
        )
        mesh_auto = {
            "Extremely fine": 1,
        ...
        assert mesh_structure in mesh_auto, (
            f"Predefined mesh type '{mesh_structure}' is not supported/recognised."
        )
        self._model.java.component("comp1").mesh("mesh1").feature("ftet10").feature(
            "size1"
        ).set("hauto", jtypes.JInt(mesh_auto[mesh_structure]))
        if mesh_structure == "Custom":
            self._model.java.component("comp1").mesh("mesh1").feature("ftet10").feature(
                "size1"
            ).set("custom", jtypes.JBoolean(True))
            if "minElementSize" in kwargs:
                ...
                self._model.java.component("comp1").mesh("mesh1").feature(
                    "ftet10"
                ).feature("size1").set("hmin", jtypes.JDouble(kwargs["minElementSize"]))
            if "maxElementSize" in kwargs:
                ...
                self._model.java.component("comp1").mesh("mesh1").feature(
                    "ftet10"
                ).feature("size1").set("hmax", jtypes.JDouble(kwargs["maxElementSize"]))
            kwargs
        if not skip_meshing:
            ...
            for cur_physic in self._cur_physics:
                ...
                if isinstance(cur_sim._sub_study, list):
                    for cur_substudy in cur_sim._sub_study:
                        self._model.java.study(cur_sim._study).feature(
                            cur_substudy
                        ).activate(cur_physic, cur_active)
                else:
                    self._model.java.study(cur_sim._study).feature(
                        cur_sim._sub_study
                    ).activate(cur_physic, cur_active)
    ...
    def save(self, file_name):
        self._model.java.component("comp1").view("view1").set(
            "transparency", jtypes.JBoolean(True)
        )  # Just a nice helpful feature to stop one extra button click...

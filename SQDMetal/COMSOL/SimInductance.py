...
class COMSOL_Simulation_Inductance(COMSOL_Simulation_Base):
    ...
    def _run_premesh(self):
        ...
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").feature(
            "bterm1"
        ).selection().named(self._loop_closure_start_end[0])
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").create(
            "bgnd1", "BoundaryGround", 1
        )
        self.jc.component("comp1").physics(self.phys_ecis).feature("csh1").feature(
            "bgnd1"
        ).selection().named(self._loop_closure_start_end[1])
        ...
        self.jc.component("comp1").physics(self.phys_mf).feature(
            "mi2"
        ).selection().named(f"geom1_{self._sel_loop_close}")
    ...
    def set_return_loop_on_route(self, qObjName, **kwargs):
        ...
        metal_polys_all = gsdf.loc[
            (gsdf["component"] == comp_id) & (~gsdf["subtract"])
        ]["geometry"].iloc[0]
        #
        unit_conv = kwargs.get("unit_conv", QUtilities.get_units(self.model.design))
        metal_polys_all = shapely.affinity.scale(
            metal_polys_all, xfact=unit_conv, yfact=unit_conv, origin=(0, 0)
        )
        ...
        self._loop_closure_start_end = (
            self.model._create_boundary_selection_sphere(
                sel_feed_rad, *start_pt, is_edge=True
            ),
            self.model._create_boundary_selection_sphere(
                sel_feed_rad, *end_pt, is_edge=True
            ),
        )

        cur_sel_index = len(self.model._conds)
        self._sel_loop_close = self.model._setup_selection_boundaries(
            cur_sel_index, select_obj_name, sel_3D_prefix="selLoopClose"
        )

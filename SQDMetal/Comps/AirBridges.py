...
class AirBridgeCPW(QComponent):
    ...
    def make(self):
        ...
        assert p.pathObj != "", (
            "Must provide a valid component name for the option: pathObj."
        )

        final_pts, normals, width, gap, total_dist = QUtilities.calc_points_on_path(
            [], self._design, self.options.pathObj
        )
        dists = np.arange(
            p.dist_air_bridges_ends,
            total_dist - p.dist_air_bridges_ends,
            p.dist_air_bridges,
        )
        if total_dist - (dists[-1] + p.dist_air_bridges) > p.dist_air_bridges_ends:
            dists = np.concatenate([dists, [dists[-1] + p.dist_air_bridges]])
        final_pts, normals, width, gap, total_dist = QUtilities.calc_points_on_path(
            dists, self._design, self.options.pathObj
        )
        assert gap > 0, (
            f"The component {self.options.pathObj} does not appear to be a valid CPW with a finite trace width and finite trace gap."
        )
        ...
        assert p.layer_bridge != p.layer, (
            "The second air bridge layer must be different to the first pad layer."
        )
        ...
        for m in range(final_pts.shape[0]):
            ...
            iBar = [
                (
                    width * 0.5 + gap + p.pad_gap + p.bridge_pad_gap,
                    -p.bridge_width * 0.5,
                ),
                (
                    width * 0.5 + gap + p.pad_gap + p.bridge_pad_gap,
                    -p.bridge_pad_length * 0.5,
                ),
                (
                    width * 0.5
                    + gap
        ...
        self.add_qgeometry(
            "poly", dict(pads=shapely.MultiPolygon(airbridge_pads)), layer=p.layer
        )
        self.add_qgeometry(
            "poly",

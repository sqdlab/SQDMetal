...
class ResonatorMeander(QComponent):
    ...
    def make(self):
        ...
        self.add_qgeometry(
            "path", {"trace": lePath}, width=p.trace_width, fillet=r, layer=p.layer
        )
        self.add_qgeometry(
            "path",
            {"trace_gap": lePath},
    ...
    def draw_resonator(l, L, r, wid_constr, f, start_left, trace_width, x, y, angle):  # noqa: E741
        ...
        if r > 0 and wid_constr > 0:
            ...
            smax = wid_constr - 2 * r
            N = np.ceil(
                (l - 3 * f - smax - 2 * np.pi * r + 2 * r) / (np.pi * r + f + smax)
            )
            N = int(N)
            assert N > 0, (
                "Constraints on meander cannot be satisfied. Try reducing fillet_padding or constr_radius. Otherwise try increasing constr_width_max."
            )
            ...
            if N > 0:
                ...
                resid = (L - L0) * 0.5
                s = (
                    l - 2 * resid - 3 * f - 2 * np.pi * r + 2 * r - N * (np.pi * r + f)
                ) / (1 + N)
            else:
                assert N == 0 and L == l, (
                    "Constraints on meander for this given point-to-point distance cannot be satisfied. Try reducing fillet_padding or constr_radius."
                )
                resid = 0
            ...
            if Nmax > 0:
                ...
                assert best_width > 0, (
                    "Constraints on meander cannot be satisfied for some reason. Try increasing wid_constr or decreasing fillet_padding."
                )
                resid, s, r, N = cur_resid_s_r_N
            else:
                assert Nmax == 0 and L == l, (
                    "Constraints on meander for this given point-to-point distance cannot be satisfied."
                )
                resid = 0
        ...
        if N == 0:
            ...
            assert s / 2 > r, (
                "Constraints on meander cannot form a curved fillet/radius on the meander. The width and radius combination make it too narrow."
            )
...
class ResonatorMeanderPin(QComponent):
    ...
    def make(self):
        ...
        lePath, pin1, pin2, r = ResonatorMeander.draw_resonator(
            ...
            startPt[0],
            startPt[1],
            np.arctan2(norm[1], norm[0]) / np.pi * 180,
        )

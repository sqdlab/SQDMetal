...
class JunctionDolan(QComponent):
    ...
    def draw_junction(p):
        ...
        pad_T = [
            ...
            (
                -p.t_finger_width * 0.5,
                -p.t_pad_length * 0.5 - t_finger_length - p.t_pad_extra,
            ),  # 6
            (
                p.t_finger_width * 0.5,
                -p.t_pad_length * 0.5 - t_finger_length - p.t_pad_extra,
        ...
        pad_Fork = [
            (
                p.finger_length + p.prong_length + p.fork_pad_size + len_stem,
                p.stem_width * 0.5,
            ),
        ...
        pad_Fork[:, 0] += (
            p.t_finger_width * 0.5 + p.bridge_gap + len_stem + p.t_pad_width / 2
        )

        sim_JJ = shapely.LineString(
            [np.mean(pad_T[1:3], axis=0), np.mean([pad_Fork[-1], pad_Fork[0]], axis=0)]
        )
...
class JunctionManhattan(QComponent):
    ...
    def make(self):
        ...
        if p.target_comp:
            start_point = self.design.components[self.options.target_comp].pins[
                self.options.target_pin
            ]
        ...
        electrode1 = ShapelyEx.fuse_polygons_threshold(
            [
                draw.rectangle(
                    p.width, p.length + p.extension, 0, -(p.length - p.extension) / 2
                ),  # main electrode
                draw.rectangle(
                    p.taper_width1,  # taper 1
                    p.length * 0.9,
                    0,
                    (-p.length - p.length * 0.1) / 2,
                ),
                draw.rectangle(
                    2 * p.taper_width1,  # taper 2
                    p.length * 0.6,
                    0,
                    (-p.length - p.length * 0.4) / 2,
                ),  # extension taper 1
        ...
        # draw the first electrode as a rectangle and add the tapering rectangles
        electrode2 = ShapelyEx.fuse_polygons_threshold(
            [
                draw.rectangle(
                    width2, length2 + extension2, 0, -(length2 - extension2) / 2
                ),
        ...
        if p.four_legs:
            # draw junction area
            junction = draw.rectangle(p.jj_width, jj_width2, 0, 0)
        else:
        ...
        extension_pin1 = draw.rotate_position(
            extension_pin1, p.orientation, start_point
        )
        extension_pin2 = draw.rotate_position(
            extension_pin2, p.orientation, start_point
        )
        ...
        self.add_pin(
            "extension_pin1", extension_pin1.coords, width=p.width, input_as_norm=True
        )
        self.add_pin(
            "extension_pin2", extension_pin2.coords, width=p.width, input_as_norm=True
        )
...
class JunctionDolanAsymmetric(QComponent):
    ...
    def draw_junction(p):
        ...
        pad_Fork = [
            ...
            (
                p.finger_length,
                (p.squid_width - p.prong_width_top + p.finger_width_top) * 0.5,
            ),
            ...
            (
                p.finger_length,
                (p.squid_width - p.prong_width_top - p.finger_width_top) * 0.5,
            ),  # play with this finger width and prong width
            ...
            (
                ...
                -p.squid_width * 0.5 + p.prong_width_bottom,
            ),
            (p.finger_length, -p.squid_width * 0.5 + p.prong_width_bottom),
            (
                p.finger_length,
                (-p.squid_width + p.prong_width_bottom + p.finger_width_bottom) * 0.5,
            ),

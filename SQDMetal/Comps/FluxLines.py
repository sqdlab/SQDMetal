...
class FluxLineTPin(QComponent):
    ...
    def make(self):
        ...
        jointPin = self._design.components[self.options.ref_comp].pins[
            self.options.ref_pin
        ]
        ...
        self.add_qgeometry("poly", dict(flTjunc=pad_T), layer=p.layer)

        # subtracts out ground plane on the layer it's on
        self.add_qgeometry("poly", dict(padGap=pad_gap), subtract=True, layer=p.layer)

...
class InductorMeander(QComponent):
    ...
    def make(self):
        ...
        self.add_qgeometry("poly", dict(inductor=poly), layer=p.layer)
    ...
    def _draw_inductor(p):
        ...
        meander = [
            (0, 0),
            (0, p.meander_width * 0.5),
            (p.meander_spacing, p.meander_width * 0.5),
        ]
        ...
        poly = shapely.LineString(meander).buffer(
            p.track_width * 0.5, join_style=2, cap_style=2
        )
        ...
        padGap = shapely.Polygon(
            [
                (mean_start_x, mean_stop_y),
                (mean_start_x, -mean_stop_y),
                (mean_stop_x, -mean_stop_y),
                (mean_stop_x, mean_stop_y),
            ]
        )
        if p.init_gap == 0:
            init_stop_y = mean_stop_y
        padGap1 = shapely.Polygon(
            [
                (0, init_stop_y),
                (0, -init_stop_y),
                (mean_start_x, -init_stop_y),
                (mean_start_x, init_stop_y),
            ]
        )
        padGap2 = shapely.Polygon(
            [
                (mean_stop_x, init_stop_y),
                (mean_stop_x, -init_stop_y),
                (len_trace, -init_stop_y),
                (len_trace, init_stop_y),
            ]
        )
...
class InductorMeanderPinPin(QComponent):
    """Draws a meandering inductor with sharp corners as defined by the meander width, spacing and number
    of long sections (see 10.2298/sjee0403057s).

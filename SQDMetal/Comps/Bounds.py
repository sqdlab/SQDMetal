...
class BoundRectangle(QComponent):
    ...
    default_options = Dict(bnd_objects="", is_ground_cut=True)
    ...
    def make(self):
        ...
        assert isinstance(obj_names, (list, tuple)), (
            "Must give bnd_objects as a list of strings."
        )
        ...
        self.add_qgeometry(
            "poly", dict(bndRect=poly), subtract=p.is_ground_cut, layer=p.layer
        )
...
class BoundGroundShield(QComponent):
    ...
    def make(self):
        ...
        for cur_excl_pin in p.exclude_pins:
            comp_name, pin_name, dist_excl = (
                cur_excl_pin  # The parser already converts units on dist_excl...
            )
        ...
        #
        self.add_qgeometry("poly", polyDict, layer=p.layer)
    ...
    def make(self):
        ...
        for m, cur_poly in enumerate(gap_polys):
            ...
            for cur_ext_coord in le_ext_coords:
                pt1, pt2 = shapely.ops.nearest_points(
                    metal_polys, shapely.Point(*cur_ext_coord)
                )
                pt1 = np.array([pt1.x, pt1.y])
                cur_coord = np.array(cur_ext_coord)
                new_ext_coords.append(
                    p.gnd_to_gap_scale * (cur_coord - pt1) + cur_coord
                )
            #
            leInteriors = []
            for cur_interior in cur_poly.interiors:
                ...
                for cur_int_coord in le_int_coords:
                    pt1, pt2 = shapely.ops.nearest_points(
                        metal_polys, shapely.Point(*cur_int_coord)
                    )
                    pt1 = np.array([pt1.x, pt1.y])
                    cur_coord = np.array(cur_int_coord)
                    new_int_coords.append(
                        p.gnd_to_gap_scale * (cur_coord - pt1) + cur_coord
                    )

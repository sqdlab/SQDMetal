...
class BandageRectPin(QComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert "target_comp" in options, (
            "Must provide a target component via 'target_comp'."
        )
        assert "target_pin" in options, (
            "Must provide a target component pin via 'target_pin'."
        )
    ...
    def make(self):
        ...
        startPt = start_point["middle"] + (
            p.offset_distance * np.cos(p.offset_direction / 180 * np.pi),
            p.offset_distance * np.sin(p.offset_direction / 180 * np.pi),
        )
        ...
        self.add_qgeometry("poly", dict(bandaid=bandaid), layer=p.layer)
...
class BandageTaperedPin(QComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not options.get("ignore_slope_check"):
            ...
            assert computed_slope < 0.4, (
                f"Slope {computed_slope:.2f} is too steep! Adjust taper dimensions for the slope to be less than 0.4."
            )
    ...
    def make(self):
        ...
        bandaid = bandaid.buffer(p.fillet_radius, join_style=2, cap_style=3).buffer(
            -p.fillet_radius,
            cap_style=1,
            join_style=1,
            mitre_limit=2.0,

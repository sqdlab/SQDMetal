...
class CapacitorInterdigital(QComponent):
    ...
    def make(self):
        ...
        pad1, pad2, padGap, pin1, pin2 = CapacitorInterdigital._draw_capacitor(
            p, self._design
        )

        # Adds the object to the qgeometry table
        self.add_qgeometry("poly", dict(pad1=pad1, pad2=pad2), layer=p.layer)

        # subtracts out ground plane on the layer it's on
        self.add_qgeometry("poly", dict(padGap=padGap), subtract=True, layer=p.layer)
    ...
    def _draw_capacitor(p, design, discard_leads=False):
        ...
        if discard_leads:
            startX = (
                len_trace * 0.5 - len_fings_plus_gap * 0.5 - p.len_flat - p.len_diag
            )
        pad1a = [
        ...
        for m in range(num_fings_1):
            ...
            fing_coords1 += [
                (cur_x, cur_y),
                (cur_x + p.fing_len, cur_y),
                (cur_x + p.fing_len, cur_y + p.fing_wid),
                (cur_x, cur_y + p.fing_wid),
                (cur_x, cur_y + p.fing_wid * 2 + p.fing_wid_gap * 2),
            ]
        ...
        if p.side_gap_uniform:
            max_extent = max(
                p.cpw_width * 0.5 + gap_cpw_line, cap_width * 0.5 + gap_cpw_cap
            )
        ...
        padGap = np.array(
            [
                (
                    len_trace * 0.5 - len_fings_plus_gap * 0.5 - p.len_flat,
                    cap_width * 0.5 + gap_cpw_cap,
                ),
                (
                    len_trace * 0.5
                    - len_fings_plus_gap * 0.5
                    - p.len_flat
                    - p.len_diag,
                    p.cpw_width * 0.5 + gap_cpw_line,
                ),
        ...
        polys = draw.rotate(
            polys,
            np.arctan2(p.end_y - p.pos_y, p.end_x - p.pos_x),
            origin=(0, 0),
            use_radians=True,
        )
...
class CapacitorGap(QComponent):
    ...
    def make(self):
        ...
        pad1, pad2, padGap1, padGap2, pin1, pin2, polyBndMid = (
            CapacitorGap._draw_capacitor(p, self._design)
        )
        ...
        # subtracts out ground plane on the layer it's on
        self.add_qgeometry(
            "poly", dict(padGap1=padGap1, padGap2=padGap2), subtract=True, layer=p.layer
        )
    ...
    def _draw_capacitor(p, design, discard_leads=False):
        ...
        pad1 = [
            ...
            (0, -p.cpw_width * 0.5),
            (
                len_trace * 0.5 - p.cap_gap * 0.5 - p.len_flat - p.len_diag,
                -p.cpw_width * 0.5,
            ),
...
class CapacitorGapPinPin(QComponent):
    ...
    def make(self):
        ...
        pad1, pad2, padGap1, padGap2, pin1, pin2, polyBndMid = (
            CapacitorGap._draw_capacitor(p, self._design, True)
        )
...
class CapacitorGapMeander(QComponent):
    ...
    def make(self):
        ...
        dict_conds, dict_gaps, pins = CapacitorGapMeander._draw_capacitor(
            p, self._design
        )
    ...
    def _draw_capacitor(p, design, discard_leads=False):
        ...
        mainG = [
            (
                len_trace * 0.5 - p.cap_length * 0.5 - p.init_pad,
                p.cap_width * 0.5 + gap_cpw_cap,
            ),
            (
                len_trace * 0.5 - p.cap_length * 0.5 - p.init_pad,
                -p.cap_width * 0.5 - gap_cpw_cap,
        ...
        lePath, pin1, pin2, r = ResonatorMeander.draw_resonator(
            l,
            L,
            r,
            wid_constr,
            f,
            p.mean_gnd_start_left,
            p.mean_gnd_width,
            len_trace * 0.5,
            -p.cap_width * 0.5,
            90,
        ...
        if not discard_leads:
            pad1 = [
                ...
                (0, -p.cpw_width * 0.5 + p.offset_lead1),
                (
                    len_trace * 0.5 - p.cap_length * 0.5,
                    -p.cpw_width * 0.5 + p.offset_lead1,
                ),
                (
                    len_trace * 0.5 - p.cap_length * 0.5,
                    p.cpw_width * 0.5 + p.offset_lead1,
            ...
            pad2 = [
                (
                    len_trace * 0.5 + p.cap_length * 0.5,
                    p.cpw_width * 0.5 + p.offset_lead2,
                ),
                (
                    len_trace * 0.5 + p.cap_length * 0.5,
                    -p.cpw_width * 0.5 + p.offset_lead2,
            ...
            pad1G = [
                ...
                (
                    len_trace * 0.5 - p.cap_length * 0.5,
                    -p.cpw_width * 0.5 - gap_cpw_line + p.offset_lead1,
                ),
                (
                    len_trace * 0.5 - p.cap_length * 0.5,
                    p.cpw_width * 0.5 + gap_cpw_line + p.offset_lead1,
            ...
            pad2G = [
                (
                    len_trace * 0.5 + p.cap_length * 0.5,
                    p.cpw_width * 0.5 + gap_cpw_line + p.offset_lead2,
                ),
                (
                    len_trace * 0.5 + p.cap_length * 0.5,
                    -p.cpw_width * 0.5 - gap_cpw_line + p.offset_lead2,
        ...
        lePathG = QiskitShapelyRenderer.get_rendered_path_poly(
            design,
            lePath.coords,
            p.mean_gnd_width + p.mean_gnd_gap * 2,
            p.mean_constr_radius,
            p.mean_turn_resolution,
        )
        lePath = QiskitShapelyRenderer.get_rendered_path_poly(
            design,
            lePath.coords,
            p.mean_gnd_width,
            p.mean_constr_radius,
        ...
        mean_path1 = ShapelyEx.rectangle(
            len_trace * 0.5 - p.mean_gnd_width * 0.5,
            -p.cap_width * 0.5,
            len_trace * 0.5 + p.mean_gnd_width * 0.5,
            -p.cap_width * 0.5 - gap_cpw_cap,
        )
        mean_path2 = ShapelyEx.rectangle(
            len_trace * 0.5 - p.mean_gnd_width * 0.5,
            p.cap_width * 0.5,
            len_trace * 0.5 + p.mean_gnd_width * 0.5,
            p.cap_width * 0.5 + gap_cpw_cap,
        ...
        if not discard_leads:
            ...
            dict_conds = dict(
                ...
                pad2=pad2,
                lePath=lePath,
                mean_path1=mean_path1,
                mean_path2=mean_path2,
            )
            dict_gaps = dict(mainG=mainG, pad1G=pad1G, pad2G=pad2G)
        else:
            main, mainG, lePath, mean_path1, mean_path2, pin1, pin2 = polys
            dict_conds = dict(
                main=main, lePath=lePath, mean_path1=mean_path1, mean_path2=mean_path2
            )
...
class CapacitorGapMeanderPinPin(QComponent):
    ...
    def make(self):
        ...
        dict_conds, dict_gaps, pins = CapacitorGapMeander._draw_capacitor(
            p, self._design, discard_leads=True
        )
...
class CapacitorUcapGroundPin(QComponent):
    ...
    def make(self):
        ...
        if p.gnd_prong_trace_dist > 0:
            gap_to_inner_gnd_side = (
                p.prong_trace_gap - p.gnd_prong_trace - p.gnd_prong_trace_dist
            )
        else:
            gap_to_inner_gnd_side = (p.prong_trace_gap - p.gnd_prong_trace) * 0.5
        if p.gnd_pad_trace_dist > 0:
            gap_to_inner_gnd_front = (
                p.pad_trace_gap - p.gnd_pad_trace - p.gnd_pad_trace_dist
            )
        else:
        ...
        gap1 = [
            ...
            (0, -p.trace_width * 0.5 - gap_to_inner_gnd_side),
            (
                p.trace_length + gap_to_inner_gnd_front,
                -p.trace_width * 0.5 - gap_to_inner_gnd_side,
            ),
            (
                p.trace_length + gap_to_inner_gnd_front,
                p.trace_width * 0.5 + gap_to_inner_gnd_side,
        ...
        pad2 = [
            (
                p.trace_length + p.pad_trace_gap + p.pad_thickness,
                p.trace_width * 0.5 + p.prong_trace_gap + p.prong_width,
            ),
            (
                p.trace_length + p.pad_trace_gap - p.prong_length,
                p.trace_width * 0.5 + p.prong_trace_gap + p.prong_width,
            ),
            (
                p.trace_length + p.pad_trace_gap - p.prong_length,
                p.trace_width * 0.5 + p.prong_trace_gap,
            ),
            (p.trace_length + p.pad_trace_gap, p.trace_width * 0.5 + p.prong_trace_gap),
            (
                p.trace_length + p.pad_trace_gap,
                -p.trace_width * 0.5 - p.prong_trace_gap,
            ),
            (
                p.trace_length + p.pad_trace_gap - p.prong_length,
        ...
        gap2 = [
            (
                p.trace_length + p.pad_trace_gap + p.pad_thickness + p.gap_back,
                p.trace_width * 0.5 + p.prong_trace_gap + p.prong_width + p.gap_side,
            ),
            (
                p.trace_length + p.pad_trace_gap - p.prong_length - p.gap_front,
                p.trace_width * 0.5 + p.prong_trace_gap + p.prong_width + p.gap_side,
            ),
            (
                p.trace_length + p.pad_trace_gap - p.prong_length - p.gap_front,
                p.trace_width * 0.5 + p.prong_trace_gap - gap_to_inner_gnd_side,
            ),
            (
                p.trace_length + p.pad_trace_gap - gap_to_inner_gnd_front,
                p.trace_width * 0.5 + p.prong_trace_gap - gap_to_inner_gnd_side,
            ),
            (
                p.trace_length + p.pad_trace_gap - gap_to_inner_gnd_front,
        ...
        if p.swap_direction:
            ...
            polys = draw.translate(
                polys, -p.trace_length - p.pad_trace_gap - p.pad_thickness
            )
        ...
        polys = draw.rotate(
            polys,
            np.arctan2(startPtNorm[1], startPtNorm[0]),
            origin=(0, 0),
            use_radians=True,
        )
        ...
        # subtracts out ground plane on the layer it's on
        self.add_qgeometry(
            "poly", dict(padGap1=gap1, padGap2=gap2), subtract=True, layer=p.layer
        )
...
class CapacitorProngPin(QComponent):
    ...
    def make(self):
        ...
        gap = [
            (
                p.pin_gap + p.pad_thickness + p.gap_back,
                -wid * 0.5 - p.pin_gap_side - p.prong_width - p.gap_side,
            ),
            (
                p.pin_gap + p.pad_thickness + p.gap_back,
                wid * 0.5 + p.pin_gap_side + p.prong_width + p.gap_side,
            ...
            (
                p.pin_gap - p.prong_length - p.gap_front,
                wid * 0.5 + p.pin_gap_side + p.prong_width + p.gap_side,
            ),
            (
                p.pin_gap - p.prong_length - p.gap_front,
        ...
        pin = shapely.LineString(
            [
                (
                    p.pin_gap + p.pad_thickness,
                    wid * 0.5 + p.pin_gap_side + p.prong_width,
                ),
                (
                    p.pin_gap + p.pad_thickness,
                    -wid * 0.5 - p.pin_gap_side - p.prong_width,
        ...
        # Adds the object to the qgeometry table
        self.add_qgeometry("poly", dict(pad=pad), layer=p.layer)

        # subtracts out ground plane on the layer it's on
        self.add_qgeometry("poly", dict(padGap=gap), subtract=True, layer=p.layer)

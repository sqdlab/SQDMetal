...
class PVD_Shadows:
    ...
    def update_pvd_profiles(self):
        ...
        cur_dict = parse_value(
            self.design.chips[self._chip_name].size,
            self.design.chips[self._chip_name].size.keys(),
        )
        self._chip_bounds = [
            (
                (cur_dict["center_x"] - cur_dict["size_x"] / 2) * unit_conv,
                (cur_dict["center_y"] - cur_dict["size_y"] / 2) * unit_conv,
            ),
            (
                (cur_dict["center_x"] + cur_dict["size_x"] / 2) * unit_conv,
                (cur_dict["center_y"] - cur_dict["size_y"] / 2) * unit_conv,
        ...
        if "evaporations" in self.design.chips[self._chip_name]:
            for cur_layer in self.design.chips[self._chip_name].evaporations.keys():
                cur_dict = parse_value(
                    self.design.chips[self._chip_name].evaporations[cur_layer],
                    self.design.chips[self._chip_name].evaporations[cur_layer].keys(),
                )
                ...
                self.evap_profiles[cur_layer_ind] = self._get_cur_pvd_profile(
                    cur_dict["bottom_layer"] * unit_conv,
    ...
    def _get_cur_pvd_profile(
        self, thickness_L, thickness_U, undercut, pvd_infos, unit_conv
    ):
        ret_list = []
        for cur_pvd_info in pvd_infos:
            ...
            #
            cur_profile = {}
            cur_profile["leading_dist"] = (thickness_L + thickness_U) * np.tan(
                angle_theta
            )
            cur_profile["tailing_dist"] = (thickness_L) * np.tan(angle_theta)
            cur_profile["tan_theta"] = np.tan(angle_theta)
            cur_profile["cur_evap_dir"] = -np.array(
                [np.cos(angle_phi), np.sin(angle_phi)]
            )
    ...
    def plot_all_layers(self, mode="separate", plot_overlap=False, **kwargs):
        ...
        for layer_id in layer_ids:
            ...
            if layer_id in self.evap_profiles:
                ...
                leShadows = self.get_all_shadows(
                    metal_polys,
                    layer_id,
                    mode,
                    list_evap_metals=new_metal_layers,
                    **kwargs,
                )
        ...
        steps = [
            name.split(",")[1].strip() if "," in name else "Mask" for name in names
        ]

        gdf = gpd.GeoDataFrame(
            {"names": names, "layers": layers, "steps": steps}, geometry=plot_polys
        )
        gdf_filt = gdf[
            (gdf["steps"] != "Mask") & (~gdf["layers"].str.contains("Mask"))
        ]  # ignore mask region to find the right overlap regions

        fig, ax = plt.subplots(1)
        unique_layers = gdf_filt["layers"].unique()
        unique_steps = gdf_filt["steps"].unique()

        assert len(unique_layers) == 2 or (
            len(unique_layers) == 1 and len(unique_steps) == 2
        ), (
        ...
        if plot_overlap:
            ...
            if len(unique_layers) == 2:
                ...
                overlap = gpd.overlay(
                    layer1, layer2, how="intersection"
            ...
            total_overlap_area = (
                overlap.geometry.area.sum() * 1e12
            )  # calculate the sum of area of the overlapping regions
    ...
    def plot_layer(self, layer_id, mode="separate", **kwargs):
        ...
        fig, ax = plt.subplots(1)
        gdf.plot(
            ax=ax, column="names", cmap="jet", alpha=0.2, categorical=True, legend=True
        )
    ...
    def get_shadow_largest_interior_for_component(
        self, qObj_name, mode="separate", **kwargs
    ):
    ...
    def get_all_shadows(self, cur_poly, layer_id, mode="merge", **kwargs):
        list_evap_metals = kwargs.get(
            "list_evap_metals", [(0, shapely.Polygon(self._chip_bounds))]
        )
        ...
        for dict_evap_params in self.evap_profiles[layer_id]:
            profiles += [
                self._get_poly_shadow(cur_poly, dict_evap_params, list_evap_metals)
            ]
            list_evap_metals = self._add_metallic_layer(
                list_evap_metals, profiles[-1], dict_evap_params["metal_thickness"]
            )
        if mode == "merge":
            ...
            assert "layer_trim_length" in kwargs, (
                "Must supply layer_trim_length if using mode: separate_delete_below."
            )
            trim_len = kwargs.get("layer_trim_length")
            for m in range(0, len(profiles) - 1):
                profiles[m] = profiles[m].difference(
                    profiles[m + 1].buffer(trim_len, join_style=2, cap_style=3)
                )
            return profiles
            ...
            for m in range(0, len(profiles)):
                for n in range(m + 1, len(profiles)):
                    intersec = shapely.intersection(profiles[m], profiles[n]).buffer(
                        trim_len, join_style=2, cap_style=3
                    )
    ...
    def _get_poly_shadow(self, cur_poly, dict_evap_params, list_evap_metals):
        ...
        allowed_region = self.get_allowed_region(
            cur_poly, undercut
        )  # Mitre joint for sharp corners...
        ...
        light_outlines = gpd.geoseries.GeoSeries(
            [x[0] for x in light_profiles]
        ).exterior.unary_union
            ...
            (
                ...
                sum(
                    [
                        ...
                        if shapely.intersection(x[0], new_polys[y]).area
                        > min_area_thresh
                    ]
                ),
            )
            for y in range(len(new_polys))
        ]
        # Select regions that are lit
        total_light = [x[0] for x in region_sums if x[1] > 0]

        final_calc_metals = self.process_geometry(
            shapely.intersection(
                shapely.geometry.multipolygon.MultiPolygon(total_light), allowed_region
            )
        )
        ...
        for m in range(len(list_evap_metals) - 1, 0, -1):
            ...
            for n in range(m - 1, -1, -1):
                ...
                for cur_line_seg in lineSegs:
                    allowed_polys[n] = self._subtract_shadows(
                        allowed_polys[n], cur_line_seg, cur_evap_dir, dist_offset
                    )
        ...
        return self.process_geometry(
            shapely.intersection(allowed_polys, final_calc_metals)
        )
    ...
    def process_geometry(self, geom):
        ...
        if isinstance(geom, shapely.geometry.collection.GeometryCollection):
            ret_poly = shapely.unary_union(
                [
                    y
                    for y in geom.geoms
                    if (
                        isinstance(y, Polygon)
                        or isinstance(y, shapely.geometry.multipolygon.MultiPolygon)
                        and y.area > 1e-25
                    )
                ]
            )
        elif isinstance(geom, shapely.geometry.multipolygon.MultiPolygon):
            ret_poly = shapely.geometry.multipolygon.MultiPolygon(
                [y for y in geom.geoms if y.area > 1e-25]
            )
    ...
    def get_line_segments(self, poly):
        ...
        if isinstance(poly, shapely.geometry.multipolygon.MultiPolygon):
            for x in poly.geoms:
                ...
                extSegs += [
                    [cur_ext_coords[m - 1], cur_ext_coords[m]]
                    for m in range(1, len(cur_ext_coords))
                ]
                ...
                for y in cur_interiors:
                    ...
                    intSegs += [
                        [cur_int_coords[m - 1], cur_int_coords[m]]
                        for m in range(1, len(cur_int_coords))
                    ]
        else:
    ...
    def get_junction_area(self, qObj_name=None, layer_id=None, **kwargs):
        ...
        if qObj_name is not None:
            ...
            layer_id = int(self.design.components[qObj_name].options.layer)
            if layer_id in self.evap_profiles:
                intersec = self.get_all_shadows(
                    metal_polys, layer_id, "separate_plot_intersections", **kwargs
                )
            else:
                print("Component does not have evap profile")
            print(
                f"Expected junction area for {qObj_name}: {intersec[0].area * 1e12} um^2"
            )
            ...
            if layer_id in self.evap_profiles:
                ...
                print("Component does not have evap profile")
            print(
                f"Expected junction area for layer {layer_id}: {intersec[0].area * 1e12} um^2"
            )
            return intersec[0].area * 1e12
    ...
    def _add_metallic_layer(
        self, cur_metal_layers, new_metal_layer, new_metal_layer_height
    ):

...
class QiskitShapelyRenderer(QMplRenderer):
    ...
    def render_poly(
        self, table: pd.DataFrame, ax, subtracted: bool = False, extra_kw: dict = None
    ):
    ...
    def get_net_coordinates(self, resolution=4):
        ...
        if len(self.dfs) == 0:
            return gpd.GeoDataFrame(
                columns=["component", "name", "geometry", "layer", "subtract"]
            )
        ...
        for index, row in df_jjs.iterrows():
            for index_cands, row_cands in gsdf.iterrows():
                if (
                    row["component"] == row_cands["component"]
                    and row["name"] == row_cands["name"]
                ):
    ...
    @staticmethod
    def get_rendered_path_poly(
        design, path_coords, path_width, fillet_radius, resolution=4
    ):
        ...
        return lePath.buffer(
            distance=path_width / 2,

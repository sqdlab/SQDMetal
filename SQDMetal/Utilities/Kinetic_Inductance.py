...
class Kinetic_Inductance:
    ...
    def calculate_kinetic_inductance(
        self,
        coherence_len,
        delta_0,
        normal_cond,
        ...
        fermi_velocity = (
            np.pi * coherence_len * (delta_0 * q) / hbar
        )  # fermi velovity, m/s
        tau = mean_free_path / fermi_velocity  # scattering time
        delta = (
            (delta_0 * q) * np.sqrt(np.cos((np.pi / 2) * (temp / t_crit) ** 2))
        )  # temperature dependent gap energy - related to the gap energy at zero temp (delta_0)
    ...
    def calc_complex_cond(self, x, y, tt, delta_0, t_crit):
        ...
        if x < 1:
            ...
            for u in u_values:
                s3 += (
                    self.GK(0.5 + (x - 1) * u**2 * (3 - u - u), x, y, t, 3)
                    * u
                    * (1 - u)
                )
            # sl = 0j  # Reset sl
    ...
    def GK(self, e, x, y, t, k):
        ...
        if k == 1:
            ...
            th = np.tanh(e / (t + t + 0.001))
            return np.tanh((e + x) / (t + t + 0.001)) * (
                (1 + c12) / (p1 - p2 + cy) - (1 - c12) / (-p1 - p2 + cy)

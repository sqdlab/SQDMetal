...
class ShapelyEx:
    ...
    def fuse_polygons_threshold(polys, threshold=1e-12):
        ...
        new_poly = shapely.unary_union(lePolys).buffer(
            -threshold, join_style=2, cap_style=3
        )
        ...
        return new_poly.buffer(-threshold, join_style=2, cap_style=3).buffer(
            threshold, join_style=2, cap_style=3
        )
    ...
    def shapely_to_list(shapely_obj):
        if isinstance(
            shapely_obj, shapely.geometry.multipolygon.MultiPolygon
        ) or isinstance(shapely_obj, shapely.geometry.multilinestring.MultiLineString):
            return [x for x in shapely_obj.geoms]
        else:
            return [
                shapely_obj
            ]  # i.e. it's just a lonely Polygon or LineString object...
    ...
    def simplify_line_edges(coords, min_angle_deg):
        ...
        while deleted:
            ...
            for m in range(numPts):
                ...
                vec_a = np.array(
                    [
                        cur_coords[m][0] - cur_coords[m - 1][0],
                        cur_coords[m][1] - cur_coords[m - 1][1],
                    ]
                )
                vec_b = np.array(
                    [
                        cur_coords[(m + 1) % numPts][0] - cur_coords[m][0],
                        cur_coords[(m + 1) % numPts][1] - cur_coords[m][1],
                    ]
                )
                ...
                len_fac = (
                    ...
                    if vec_a_norm > vec_b_norm
                    else vec_b_norm / vec_a_norm
                )
    ...
    def simplify_poly_edges(shapely_poly, min_angle_deg):
        ...
        for cur_poly in list_shapely_polys:
            exteriors = ShapelyEx.simplify_line_edges(
                cur_poly.exterior.coords[:], min_angle_deg
            )
            interiors = [
                ShapelyEx.simplify_line_edges(cur_int.coords[:], min_angle_deg)
    ...
    def get_smallest_feature_size(shapely_poly):
        ...
        for cur_poly in lePolys:
            for m in range(1, len(cur_poly.exterior.coords)):
                cur_dist2 = (
                    cur_poly.exterior.coords[m][0] - cur_poly.exterior.coords[m - 1][0]
                ) ** 2 + (
                    cur_poly.exterior.coords[m][1] - cur_poly.exterior.coords[m - 1][1]
                ) ** 2
                smallest_dist2 = min(smallest_dist2, cur_dist2)
            for cur_int in cur_poly.interiors:
                for m in range(1, len(cur_int.coords)):
                    cur_dist2 = (
                        cur_int.coords[m][0] - cur_int.coords[m - 1][0]
                    ) ** 2 + (cur_int.coords[m][1] - cur_int.coords[m - 1][1]) ** 2
                    smallest_dist2 = min(smallest_dist2, cur_dist2)
    ...
    def get_shapely_polygon_skeleton(shapely_polyon: shapely.Polygon, **kwargs):
        def get_bisecting_rays(coords_ring: list, orient_CCW=True):
            ...
            for m in range(
                0, len(coords_ring) - 1
            ):  # Assuming repeated initial coordinate...
        ...
        def find_intersec_collapse_ray(
            shapely_poly, rays: list, overall_rays: list, overall_rays_stilts: list
        ):
            minx, miny, maxx, maxy = shapely_poly.bounds
            ...
            if lowest_dist_ind >= 0:
                if rays[lowest_dist_ind - 1]["child"]:
                    overall_rays.append(
                        (rays[lowest_dist_ind - 1]["pt"], lowest_dist_pt)
                    )
                else:
                    overall_rays_stilts.append(
                        (rays[lowest_dist_ind - 1]["pt"], lowest_dist_pt)
                    )
                if rays[lowest_dist_ind]["child"]:
                    ...
                    overall_rays_stilts.append(
                        (rays[lowest_dist_ind]["pt"], lowest_dist_pt)
                    )
                if lowest_dist_ind == 0:
                    ray1 = rays.pop(-1)
                    ray2 = rays.pop(0)
                ...
                child_vec = (
                    rays[min_targ_ind]["next_edge"]
                    + rays[min_targ_ind - 1]["prev_edge"]
                ) / 2
        ...
        while len(rays) > 2:
            ...
            res = find_intersec_collapse_ray(
                shapely_poly, rays, overall_rays, overall_rays_stilts
            )
            ...
            if debug_plots:
                ...
                for cur_ray in rays:
                    ...
                    plt.plot(
                        [pt[0], pt[0] + vec[0] * vecMag],
                        [pt[1], pt[1] + vec[1] * vecMag],
                        "r",
                    )
                for cur_ray in overall_rays:
                    plt.plot(
                        [cur_ray[0][0], cur_ray[1][0]],
                        [cur_ray[0][1], cur_ray[1][1]],
                        "b-",
                    )
        ...
        if plot_result:
            ...
            for cur_ray in overall_rays:
                plt.plot(
                    [cur_ray[0][0], cur_ray[1][0]], [cur_ray[0][1], cur_ray[1][1]], "b-"
                )
            for cur_ray in overall_rays_stilts:
                plt.plot(
                    [cur_ray[0][0], cur_ray[1][0]], [cur_ray[0][1], cur_ray[1][1]], "g-"
                )

        return {"skeleton": overall_rays, "stilts": overall_rays_stilts}

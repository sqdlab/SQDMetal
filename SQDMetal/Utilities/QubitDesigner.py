    ...
    def __init__(self, f0=None, L=None, C=None):
        if f0 is None:
            assert L is not None and C is not None, (
                "Must supply L and C if omitting f0."
            )
            self.L = L
            self.C = C
            self.f0 = 1 / (2 * np.pi * np.sqrt(L * C))
        elif L is None:
            assert f0 is not None and C is not None, (
                "Must supply f0 and C if omitting L."
            )
            self.f0 = f0
            self.C = C
            self.L = 1 / ((2 * np.pi * f0) ** 2 * C)
        elif C is None:
            assert f0 is not None and L is not None, (
                "Must supply f0 and L if omitting C."
            )
            self.f0 = f0
            self.L = L
            self.C = 1 / ((2 * np.pi * f0) ** 2 * L)
        else:
            assert np.abs(2 * np.pi * f0 - 1 / np.sqrt(L * C)) < 1e-9, (
                "System overconstrained. Just supply 2 of L, C and F0..."
            )
...
class TransmonBase:
    ...
    def parse_params(self, param_constraints):
        ...
        for m, cur_param in enumerate(default_params):
            cur_param_constraints = (
                param_constraints if cur_param in param_constraints else default_params
            )
            #
            if (
                isinstance(cur_param_constraints[cur_param], tuple)
                or isinstance(cur_param_constraints[cur_param], list)
                or isinstance(cur_param_constraints[cur_param], np.ndarray)
            ):
                cons = np.array(cur_param_constraints[cur_param])
                assert cons.size == 2, (
                    "Must provide the constraint as either a single value (equality) or a tuple (bounded region)."
                )
                constrs.append((cons[0], cons[1]))
                x0.append(0.5 * (cons[0] + cons[1]))
            else:
                constrs.append(
                    (cur_param_constraints[cur_param], cur_param_constraints[cur_param])
                )
    ...
    def chk_constr_resp(self, constr_name, x, constrs, mps):
        ...
        if isinstance(constrs, (list, tuple, np.ndarray)):
            ...
            respected = (
                leVal / constrs[0] - 1 >= -1e-6 and leVal / constrs[1] - 1 <= 1e-6
            )
        else:
...
class XmonDesigner(TransmonBase):
    def __init__(self, resonator):
        assert resonator.is_res_parallelLC(), (
            "Resonator must be in a parallel-coupled arrangment."
        )
    ...
    def optimise(self, param_constraints):
        ...
        fres, Cres, Lres = (
            self.resonator.get_res_frequency(),
            self.resonator.get_res_capacitance(),
            self.resonator.get_res_inductance(),
        )

        def func(x):
            return abs(
                self._chi_hertz(
                    x[mps["C_g"]], x[mps["C_J"]], x[mps["fQubit"]], fres, Cres, Lres
                )
                / x[mps["chi"]]
                - 1
            ) + abs(
                self.EJonEC(
                    self._eff_CJ(x[mps["C_J"]], x[mps["C_g"]], Cres), x[mps["fQubit"]]
                )
                / x[mps["Ej/Ec"]]
                - 1
            )
...
class FloatingTransmonDesigner(TransmonBase):
    ...
    def _eff_Cq(self, C_q1, C_q2, C_g1, C_g2):
        return (C_g1 * C_g2 + 2 * C_g2 * C_q1 + C_q1 * C_q2) / (
            C_q1 + C_q2 + C_g1 + C_g2
        )
    ...
    def _beta(self, C_q1, C_q2, C_g1, C_g2, C_J):
        return (C_g1 * C_q2 - C_g2 * C_q1) / (
            (C_q1 + C_g1) * (C_q2 + C_g2) + (C_q1 + C_q2 + C_g1 + C_g2) * C_J
        )
    ...
    def optimise(self, param_constraints):
        ...
        def func(x):
            return (
                ...
                + abs(
                    self.EJonEC(
                        self._eff_CJ(
                            self._eff_Cq(*x[1:5]) + x[5], self._eff_Cg(*x[1:5]), Cres
                        ),
                        x[mps["fQubit"]],
                    )
                    / x[mps["Ej/Ec"]]
        ...
        g = self._g_hertz(
            self._eff_Cg(*sol.x[1:5]),
            self._eff_Cq(*sol.x[1:5]) + sol.x[5],
            sol.x[0],
            Cres,
            Lres,
        )
        ...
        anh = self._anharmonicity_hertz(
            self._eff_Cg(*sol.x[1:5]), self._eff_Cq(*sol.x[1:5]) + sol.x[5], Cres
        )

        x[mps["beta"]] = self._beta(*x[1:6])
        x[mps["C_sigma"]] = self._Csigma(*x[1:6])
        x[mps["Ej/Ec"]] = self.EJonEC(
            self._eff_CJ(self._eff_Cq(*x[1:5]) + x[5], self._eff_Cg(*x[1:5]), Cres),
            x[mps["fQubit"]],
        )
        ...
        print("Qubit:")
        print(
            f"\tFrequency: {GenUtilities.add_units(sol.x[0], sigfigs)}Hz"
            + self.chk_constr_resp("fQubit", x, constrs, mps)
        )
        print(f"\tAnharmonicity: {GenUtilities.add_units(anh, sigfigs)}Hz")
        print(f"\tg: {GenUtilities.add_units(g, sigfigs)}Hz")
        print(f"\tDelta: {GenUtilities.add_units(x[mps['fQubit']] - fres, sigfigs)}Hz")
        print(f"\tchi: {GenUtilities.add_units(chi, sigfigs)}Hz")
        print(
            f"\tCq1: {GenUtilities.add_units(x[mps['C_q1']], sigfigs)}F"
            + self.chk_constr_resp("C_q1", x, constrs, mps)
        )
        print(
            f"\tCq2: {GenUtilities.add_units(x[mps['C_q2']], sigfigs)}F"
            + self.chk_constr_resp("C_q2", x, constrs, mps)
        )
        print(
            f"\tCg1: {GenUtilities.add_units(x[mps['C_g1']], sigfigs)}F"
            + self.chk_constr_resp("C_g1", x, constrs, mps)
        )
        print(
            f"\tCg2: {GenUtilities.add_units(x[mps['C_g2']], sigfigs)}F"
            + self.chk_constr_resp("C_g2", x, constrs, mps)
        )
        print(
            f"\tCJ: {GenUtilities.add_units(x[mps['C_J']], sigfigs)}F"
            + self.chk_constr_resp("C_J", x, constrs, mps)
        )
        print(
            f"\tEj/Ec: {GenUtilities.add_units(x[mps['Ej/Ec']], sigfigs, True)}"
            + self.chk_constr_resp("Ej/Ec", x, constrs, mps)
        )
        print(
            f"\tCΣ: {GenUtilities.add_units(x[mps['C_sigma']], sigfigs)}F"
            + self.chk_constr_resp("C_sigma", x, constrs, mps)
        )
        print(
            f"\tbeta: {GenUtilities.add_units(x[mps['beta']], sigfigs, True)}"
            + self.chk_constr_resp("beta", x, constrs, mps)
        )
...
if __name__ == "__main__":
    ...
    FloatingTransmonDesigner(ResonatorHalfWave(7.5e9)).optimise(
        {
            "fQubit": (1e9, 10e9),
            "C_q1": (30.862e-15),
            "C_q2": (31.974e-15),

...
class MakeGDS:
    ...
    def _shapely_to_gds(self, metals, layer):
        ...
        for cur_poly in temp_cur_metals:
            if gds_metal:
                gds_metal = gdstk.boolean(
                    gdstk.Polygon(cur_poly.exterior.coords[:]),
                    gds_metal,
                    "or",
                    layer=layer,
    ...
    def refresh(self, full_refresh=False):
        ...
        assert self.export_type in ["all", "negative", "positive"], (
            'Export type must be: "all", "negative", or "positive"'
        )

        if self.export_layers is not None:
            self.export_type = "all"
            print(
                f"Exporting layers {self.export_layers}"
            ) if self.print_statements else 0
        ...
        sx = (
            QUtilities.parse_value_length(self._design.chips["main"]["size"]["size_x"])
            / leUnits
        )
        sy = (
            QUtilities.parse_value_length(self._design.chips["main"]["size"]["size_y"])
            / leUnits
        )
        cx, cy = [
            QUtilities.parse_value_length(self._design.chips["main"]["size"][x])
            / leUnits
        ...
        chip = shapely.affinity.scale(
            chip, xfact=unit_conv, yfact=unit_conv, origin=(0, 0)
        )
        ...
        for layer in leLayers:
            ...
            metals = ShapelyEx.fuse_polygons_threshold(
                metals["geometry"].tolist(), threshold
            )
            metals = shapely.affinity.scale(
                metals, xfact=unit_conv, yfact=unit_conv, origin=(0, 0)
            )
        ...
        for cur_layer in all_layers:
            ...
            print(
                f" Added negative layer {layer + neg_layer_offset}"
            ) if self.print_statements else 0
        ...
        if (len(all_layers) == 2) and (self.export_type in ["positive", "negative"]):
            print(
                f"  Two layers detected - performing boolean operations for {self.export_type} export"
            ) if self.print_statements else 0
            self.add_boolean_layer(11, 12, "and", output_layer=0)
            self.merge_polygons_in_layer(0)
            self.cell.remove(
                *[poly for poly in self.cell.polygons if poly.layer in [11, 12]]
            )
            ...
            if self.export_type == "positive":
                ...
                gds_positive = gdstk.boolean(
                    gds_gnd_plane, gds_metal_neg, "not", layer=0
                )
        ...
        if self.export_layers is not None:
            assert isinstance(self.export_layers, list), (
                "Please pass a list of integers as 'export_layers'."
            )
            layer_list = self.get_cell_layers()
            assert set(self.export_layers).issubset(layer_list), (
                "Chosen export layers are not present in the design."
            )
            ...
            if isinstance(to_delete, list):
                ...
                self.cell.remove(
                    *[p for p in self.cell.polygons if p.layer == to_delete]
                )
    ...
    def add_boolean_layer(self, layer1_ind, layer2_ind, operation, output_layer=None):
        assert operation in ["and", "or", "xor", "not"], (
            'Operation must be: "and", "or", "xor", "not"'
        )
        if output_layer is None:
            output_layer = max([x for x in self._layer_metals]) + 1
        try:
            new_metal = gdstk.boolean(
                self._layer_metals[layer1_ind],
                self._layer_metals[layer2_ind],
                operation,
            ...
            print(
                f"Warning: Boolean operation ({layer1_ind}, {layer2_ind}) failed: one of the input layers ({layer1_ind}, {layer2_ind}) has no polygons."
            )
            return output_layer
        if self.smooth_radius > 0:
        ...
        if not new_metal:
            print(
                f"No geometry resulted from boolean operation '{operation}' between layers {layer1_ind} and {layer2_ind}."
            )
            return output_layer
    ...
    def add_text(self, text_label="", layer=0, size=300, position=None):
        ...
        assert isinstance(position, (tuple, NoneType)), (
            "Please pass the position as an (x,y) tuple"
        )
        if isinstance(position, tuple):
            for i in position:
                assert 0 <= i <= 1, (
                    "Ensure the positions are normalised to 1 (i.e. (0,0) is bottom left, (1,1) is top right)"
                )

        layer_list = self.get_cell_layers()
        if (layer in layer_list) and (layer != 0):
            print(
                "Warning: text is being added with no action argument to a (non-ground-plane) layer already containing shapes. Consider changing the layer."
            )

        # default label
        text_gds = (
            f"Made with SQDMetal {datetime.now().strftime('%Y')}."
            if text_label == ""
            else text_label
        )
        ...
        if position is None:
            text_position = (
                (self.cx - default_pos * self.sx) * unit_conv,
                (self.cy - default_pos * self.sy) * unit_conv,
            )
        else:
            text_position = (
                (self.cx + (position[0] - 0.5) * self.sx) * unit_conv,
                (self.cy + (position[1] - 0.5) * self.sy) * unit_conv,
            )
    ...
    def merge_gds_per_layer_inplace(gds_input_file, inplace=True):
        try:
            ...
            # Load the layout
            layout = pya.Layout()
            layout.read(f"{gds_input_file}.gds") if gds_input_file[
                -4:
            ] != ".gds" else layout.read(f"{gds_input_file}")
            ...
            layers_in_design = [
                layout.get_info(layer_index) for layer_index in layout.layer_indices()
            ]
            if inplace:
            ...
            print(
                "KLayout (pya) is not installed. Please install it with 'pip install klayout' to run this function."
            )
        except AttributeError as e:
            print(
                f"Attribute error: {e}. Please ensure you're using the correct KLayout API (i.e. NOT 'pip install pya' - this is a different library). Use 'pip install klayout' instead."
            )

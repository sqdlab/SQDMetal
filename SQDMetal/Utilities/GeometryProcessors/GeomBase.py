...
class GeomBase:
    ...
    def create_CPW_feed_via_point(
        self, pt_near_centre_end, len_launch, metallic_layers, ground_plane, **kwargs
    ):
        ...
        for cur_line in poly_boundary_lines:
            for m in range(len(cur_line.coords) - 1):  # Ignore last point...
                ...
                vec1 = np.array(
                    [
                        coord_1[0] - cur_line.coords[m][0],
                        coord_1[1] - cur_line.coords[m][1],
                    ]
                )
                vec2 = np.array(
                    [
                        coord_2[0] - cur_line.coords[m][0],
                        coord_2[1] - cur_line.coords[m][1],
                    ]
                )
                #
        ...
        vec_perp = np.array(
            [end_point[0] - pt_near_centre_end[0], end_point[1] - pt_near_centre_end[1]]
        )
        if is_inside:
            vec_perp = -vec_perp
        ...
        for cur_corner in corner_points:
            vec_cur_corner = np.array(
                [cur_corner[0] - end_point[0], cur_corner[1] - end_point[1]]
            )
        ...
        pad_pts_1, pad_pts_2 = (
            sorted(collinear_corners_1)[:2],
            sorted(collinear_corners_2)[:2],
        )

        assert len(pad_pts_1) == 2, (
            f"Seems that a CPW couldn't be formed. Check that the line-segment closest to the point {pt_near_centre_end} is indeed the edge of the central conductor of the CPW feed."
        )
        assert len(pad_pts_2) == 2, (
            f"Seems that a CPW couldn't be formed. Check that the line-segment closest to the point {pt_near_centre_end} is indeed the edge of the central conductor of the CPW feed."
        )

...
class GeomQiskitMetal(GeomBase):
    def __init__(self, design, **kwargs):
        ...
        if (
            ...
            self._chip_centre = [
                (self._restrict_rect[0] + self._restrict_rect[2]) * 0.5,
                (self._restrict_rect[1] + self._restrict_rect[3]) * 0.5,
                QUtilities.parse_value_length(
                    self.design.chips["main"]["size"]["center_z"]
                ),
            ]
        else:
            self._chip_size_x = QUtilities.parse_value_length(
                self.design.chips["main"]["size"]["size_x"]
            )
            self._chip_size_y = QUtilities.parse_value_length(
                self.design.chips["main"]["size"]["size_y"]
            )
            self._chip_centre = [
                QUtilities.parse_value_length(self.design.chips["main"]["size"][x])
                for x in ["center_x", "center_y", "center_z"]
            ]
            self._restrict_rect = [
                self.chip_centre[0] - 0.5 * self._chip_size_x,
                self.chip_centre[1] - 0.5 * self._chip_size_y,
                self.chip_centre[0] + 0.5 * self._chip_size_x,
                self.chip_centre[1] + 0.5 * self._chip_size_y,
            ]
        self._chip_size_z = QUtilities.parse_value_length(
            self.design.chips["main"]["size"]["size_z"]
        )
    ...
    def process_layers(self, metallic_layers, ground_plane, **kwargs):
        ...
        if not ground_plane["omit"]:
            ...
            space_polys = ShapelyEx.fuse_polygons_threshold(
                filt["geometry"].buffer(0), fuse_threshold / qm_units
            )
            space_polys = ShapelyEx.shapely_to_list(space_polys)
            space_polys = [
                shapely.affinity.scale(x, xfact=qm_units, yfact=qm_units, origin=(0, 0))
                for x in space_polys
            ]
            #
            metal_surface = shapely.geometry.box(
                self._chip_centre[0] - 0.5 * self._chip_size_x,
                self._chip_centre[1] - 0.5 * self._chip_size_y,
                self._chip_centre[0] + 0.5 * self._chip_size_x,
                self._chip_centre[1] + 0.5 * self._chip_size_y,
            )
            ground_plane_poly = shapely.difference(
                metal_surface, shapely.geometry.multipolygon.MultiPolygon(space_polys)
            )

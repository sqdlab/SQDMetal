...
class GeomGDS(GeomBase):
    ...
    def process_layers(self, metallic_layers, ground_plane, **kwargs):
        if not ground_plane["omit"]:
            print(
                "WARNING: Don't use add_ground_plane() when using GDS. It is being ignored here..."
            )
        ...
        for cur_layer in metallic_layers:
            if cur_layer["type"] == "design_layer":
                ...
                metal_polys = leCell.get_polygons(
                    layer=cur_layer["layer_id"], datatype=0
                )
                metal_polys = [
                    shapely.Polygon(cur_poly.points * leUnits)
                    for cur_poly in metal_polys
                ]
                if isinstance(self._restrict_rect, list):
                    clip_rect = [x / unit_conv for x in self._restrict_rect]
                    metal_polys = [
                        shapely.clip_by_rect(cur_poly, *clip_rect)
                        for cur_poly in metal_polys
                    ]
                metal_polys = ShapelyEx.fuse_polygons_threshold(
                    metal_polys, cur_layer["fuse_threshold"]
                )
        ...
        temp_metal_fuse = ShapelyEx.fuse_polygons_threshold(
            metal_polys_all, kwargs.get("fuse_threshold", 1e-12) / unit_conv
        )
        simplify_edge_min_angle_deg = kwargs.get("simplify_edge_min_angle_deg", -1)
        if simplify_edge_min_angle_deg >= 0:
            temp_metal_fuse = ShapelyEx.simplify_poly_edges(
                temp_metal_fuse, simplify_edge_min_angle_deg
            )

        if not isinstance(self._restrict_rect, list):
            substrate_box_bnds = temp_metal_fuse.bounds
            self._chip_size_x = (
                substrate_box_bnds[2] - substrate_box_bnds[0]
            ) * unit_conv
            self._chip_size_y = (
                substrate_box_bnds[3] - substrate_box_bnds[1]
            ) * unit_conv
            self._chip_centre = [
                (substrate_box_bnds[2] + substrate_box_bnds[0]) / 2 * unit_conv,
                (substrate_box_bnds[3] + substrate_box_bnds[1]) / 2 * unit_conv,
                0,
            ]
        else:

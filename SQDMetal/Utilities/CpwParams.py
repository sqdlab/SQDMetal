...
class CpwParams:
    ...
    def fromMaterial(cls, material_obj, h):
        ...
        if isinstance(material_obj, str):
            ...
            assert False, (
                'The first argument "material_obj" must be a Material object type or a string for the material name.'
            )
        return CpwParams(Er, h)
    ...
    def calc_impedance(tr_wid, tr_gap, er, h):
        ...
        kp_k1_on_k_k1p = (
            scipy.special.ellipk(kp**2)
            * scipy.special.ellipk(k1**2)
            / (scipy.special.ellipk(k**2) * scipy.special.ellipk(k1p**2))
        )
        ...
        z0 = (
            ...
            * 1.0
            / (
                scipy.special.ellipk(k**2) / scipy.special.ellipk(kp**2)
                + scipy.special.ellipk(k1**2) / scipy.special.ellipk(k1p**2)
            )
        )
    ...
    def get_gap_from_width(self, trace_width, target_impedance=50):
        ...
        res = scipy.optimize.minimize(
            lambda x: np.abs(
                CpwParams.calc_impedance(trace_width, x[0], er, h) - target_impedance
            ),
            x0,
            method="Nelder-Mead",
            tol=1e-6,
    ...
    def get_width_from_gap(self, trace_gap, target_impedance=50):
        ...
        res = scipy.optimize.minimize(
            lambda x: np.abs(
                CpwParams.calc_impedance(x[0], trace_gap, er, h) - target_impedance
            ),
            x0,
            method="Nelder-Mead",
            tol=1e-6,
    ...
    def get_width_from_space(self, gnd_plane_gap, target_impedance=50):
        ...
        res = scipy.optimize.minimize(
            lambda x: np.abs(
                CpwParams.calc_impedance(x[0], 0.5 * (gnd_plane_gap - x[0]), er, h)
                - target_impedance
            ),
            x0,
            method="Nelder-Mead",
